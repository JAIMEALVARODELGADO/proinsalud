<?
	...
	$bima=mysql_query("SELECT mapii.codi_map, mapii.desc_map, mapii.clas_map, detareferencia.obsv_dre, detareferencia.ccie_dre, detareferencia.cant_dre, detareferencia.numc_dre
	FROM mapii INNER JOIN detareferencia ON mapii.codi_map = detareferencia.codi_dre
	WHERE (((mapii.clas_map)='1806') AND ((detareferencia.numc_dre)='$numhisto'))");
	...
	$fin1=$i;
	$blab=mysql_query("SELECT mapii.codi_map, mapii.desc_map, mapii.clas_map, detareferencia.obsv_dre, detareferencia.ccie_dre, detareferencia.cant_dre, detareferencia.numc_dre
	FROM mapii INNER JOIN detareferencia ON mapii.codi_map = detareferencia.codi_dre
	WHERE (((mapii.clas_map)='1804') AND ((detareferencia.numc_dre)='$numhisto'))");
	...
	$fin2=$i;	
	$brem=mysql_query("SELECT mapii.codi_map, mapii.desc_map, mapii.clas_map, detareferencia.obsv_dre, detareferencia.ccie_dre, detareferencia.cant_dre, detareferencia.numc_dre
	FROM mapii INNER JOIN detareferencia ON mapii.codi_map = detareferencia.codi_dre
	WHERE (((mapii.clas_map)<>'1804') AND ((mapii.clas_map)<>'1806') AND ((detareferencia.numc_dre)='$numhisto'))");
	$i=0;
...
$pdf->SetXY(98,$fila);
		$pdf->SetFillColor($col);	
		$pdf->rect(5,$fila,205,5,F);
				$pdf->Cell(20,5,'CONTRAREFERENCIA ',0,0,L);
		//$pdf->SetDrawColor(150);
		//		$pdf->line(5,$fila-0,210,$fila-0);
		//		$pdf->line(5,$fila+5,210,$fila+5);
		$fila=$fila+5;
		$pdf->SetXY(5,$fila);
		$pdf->MultiCell(205,5,$contrare,0,J,0);
...
function titulo(&$pdf_,&$fila_,&$vec_,&$pag,$m,$pos)
{
	...
	if($m==5)
	{
		...
		{
			$pdf_->Image('img\enca_urg.JPG',1,$fila_,210,0,'','');
		}
