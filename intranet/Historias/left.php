...
//base_general();
$consultausu=mysql_query("SELECT nomb_usua,tip_usuario FROM cut WHERE ide_usua='$Gidusu_hist'");
...
<div class="screenBody" id="">
  <form action="left.php" method="post" enctype="multipart/form-data" >
    <table id="navArea" cellspacing="0" cellpadding="0" width="100%" border="0" summary="Navigation Items Area">
      <?
        $consulta=mysql_query("SELECT codi_men,descr_men FROM menu WHERE aplic_men='05' and nivel_men=1");
					...
					<?
					  if($tip_usuario=='02'){
					    $consultaopc=mysql_query("SELECT descr_men,url_men,img_men FROM menu WHERE nivel_men=2 and depen_men=$row[codi_men]");}
                           ...
                           INNER JOIN menuxusu AS mxu ON mxu.codi_men=m.codi_men AND mxu.ide_usua='$Gidusu_hist'
