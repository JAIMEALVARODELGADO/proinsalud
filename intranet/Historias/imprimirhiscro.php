<?php
	...
	$porcionesFecha = explode("-", $fechaimpre);
	$fechaActual=$porcionesFecha[0].$porcionesFecha[1].$porcionesFecha[2];
	...
	$brem=mysql_query("SELECT cups.codigo, cups.codi_cup, cups.descrip, cups.grup_cup, detareferencia.obsv_dre, detareferencia.ccie_dre, detareferencia.cant_dre,
	detareferencia.numc_dre, detareferencia.dest_dre
	FROM detareferencia INNER JOIN cups ON detareferencia.codi_dre = cups.codigo
	...
	while($rusu=mysql_fetch_array($busu))
	{
		...
		$cous=$rusu['cous_ehi'];
		$bdoc=mysql_query("select TDOC_USU, FNAC_USU, EMAI_USU, MRES_USU from usuario where CODI_USU='$cous'");
...
//------------------ Cambio 2024 -------------------			
			$correo_usuario=$rdoc['EMAI_USU'];
			$municipio_reside=$rdoc['MRES_USU'];
//--------------------------------------------------			
	...
	if($ima=='1')
	{
		...
		for($i=0;$i<2;$i++)
		{
			...
			{	
				...
				{
					if($cod1cpl!='B24X')$vardescricie=$rowcroimpre6['nom_cie10'];
				}
				$fila=$fila+4;
				$pdf->SetXY(5,$fila);
				...
				while($rowcroimpre7=mysql_fetch_array($resutcroimpre7))
				{
					...
					if($vardescod7!='B24X')$vardescricie7=$rowcroimpre7['nom_cie10'];
					$fila=$fila+4;
					$pdf->SetXY(5,$fila);
			...
			{	
				...
				{		
					...
					{
						$fila=$fila+2;
						if($desdies2!='B24X')$desd1=$rd1['nom_cie10'];
	...
	{		
		for($i=0;$i<2;$i++)
		{
			...
			{
				if($diag != 'B24X')$desd1=$rd1['nom_cie10'];
			}	
			$fila=$fila+4;		
			$pdf->SetXY(5,$fila);	
			$pdf->Cell(10,5,'Diagnosticos',0,0,L);	
				...
				{				
					...
					$bddiag=mysql_query("select * from cie_10 where cod_cie10='$desdies2'");
									while($rd1=mysql_fetch_array($bddiag))
									{
											$desd1='';
											$fila=$fila+2;
											if($desdies2 != 'B24X')$desd1=$rd1['nom_cie10'];$desd1=$rd1['nom_cie10'];
											$pdf->MultiCell($fila,3,'       ' .$desdies2.'-'.$desd1,0,L,0);				
											$fila=$pdf->GetY();
									}				
	...
	if($med=='1')
	{
		for($i=0;$i<2;$i++)
		{
			...
			while($rd1=mysql_fetch_array($bd1))
			{
				if($diag1 != 'B24X')$desd1=$rd1['nom_cie10'];
			}		
			$fila=$fila+4;		
			$pdf->SetXY(5,$fila);	
			$pdf->Cell(10,5,'Diagnosticos',0,0,L);	
			...
			else
			{
				...
				{				
					...
					while($rd1=mysql_fetch_array($bddiag))
					{
						$desd1='';
						$fila=$fila+2;
						if($desdies2!='B24X')$desd1=substr($rd1['nom_cie10'],0,20);
...
//fin
			...
			$condatBas=Mysql_query("SELECT tipoconsulta_dat, tiempocons, ultimacon_dat, fechaprogra_dat, zona_dat, oriensex_dat, afiliacion, disfisica,  dismotora,  disauditiva,  disvisual, victima, tamizado, trabajosex, conidigencia, privalibertad, consumespa, enfermental, abandonosocial, noescolarizado, victimasex, violeciafamiliar FROM cr_datoscronicos WHERE numhisto='$numhistohc'");
...
//-------------------------- Cambio 2024 ----------------------------				
				$trabajosex=$rowdatBas['trabajosex'];
				$conidigencia=$rowdatBas['conidigencia'];
				$privalibertad=$rowdatBas['privalibertad'];
				$consumespa=$rowdatBas['consumespa'];
				$enfermental=$rowdatBas['enfermental'];
				$abandonosocial=$rowdatBas['abandonosocial'];
				$noescolarizado=$rowdatBas['noescolarizado'];
				$victimasex=$rowdatBas['victimasex'];
				$violeciafamiliar=$rowdatBas['violeciafamiliar'];
//-------------------------------------------------------------------				
			}
			$discapacidadcro='';
...
//---------------------------------- Cambio 2024 ------------------------------------			
			...
			if($violeciafamiliar==1){
				$violeciafamiliar1='SI';
			}
			else {
				$violeciafamiliar1='NO';
			}
//------------------------------------------------------------------------------			
			...
			$col=220;
			$fila=6;
			if($numhistohc=='11022082240617055634')$pdf->Image('img\encabezadonew_2.JPG',3,$fila,210,0,'','');
			else $pdf->Image('img\encabezadonew.JPG',3,$fila,210,0,'','');
			$fila=$pdf->GetY();
			$numpagtot1 = 1;
			$pdf->SetXY(190,17.5);
			$pdf->SetFont('Arial','',9);
			...
			$pdf->SetXY(135,$fila);
			$pdf->Cell(75,4,utf8_decode("REGIMEN DE AFILIACION: ".$afiliacion),0,0,'L');
...
//--------------------------------- Cambio 2024 ----------------------------------------			
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(128,4,utf8_decode("CORREO ELECTRONICO: ".$correo_usuario),0,0,'L');
			$pdf->SetXY(135,$fila);
			$pdf->Cell(128,4,utf8_decode("CONTRATO:  ".$vec[5]),0,0,'L');
			
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(128,4,utf8_decode("MUNICIPIO RESIDENCIA: ".$municipio_reside),0,0,'L');
...
// ----------------------------------- CAMBIO DE 2024 -------------------------------------------
// vich			
		if($feca_cpl>'2024-05-07'){	
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(128,4,utf8_decode("TRABAJADOR SEXUAL: ".$trabajosex1),0,0,'L');
			$pdf->SetXY(60,$fila);
			$pdf->Cell(128,4,utf8_decode("CONDICIÓN DE INDIGENCIA:  ".$conidigencia1),0,0,'L');
			$pdf->SetXY(135,$fila);
			$pdf->Cell(128,4,utf8_decode("PRIVADO DE LA LIBERTAD:  ".$privalibertad1),0,0,'L'); 
			
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(128,4,utf8_decode("CONSUMO DE SPA: ".$consumespa1),0,0,'L');
			$pdf->SetXY(60,$fila);
			$pdf->Cell(128,4,utf8_decode("ENFERMEDAD MENTAL:  ".$enfermental1),0,0,'L');
			$pdf->SetXY(135,$fila);
			$pdf->Cell(128,4,utf8_decode("ABANDONO SOCIAL:  ".$abandonosocial1),0,0,'L'); 

			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(128,4,utf8_decode("NO ESCOLARIZADO: ".$noescolarizado1),0,0,'L');
			$pdf->SetXY(60,$fila);
			$pdf->Cell(128,4,utf8_decode("VICTIMA DE VIOLENCIA SEXUAL:  ".$victimasex1),0,0,'L');
			$pdf->SetXY(135,$fila);
			$pdf->Cell(128,4,utf8_decode("VIOLENCIA FAMILIAR O INTRAFAMILIAR:  ".$violeciafamiliar1),0,0,'L'); 
		}
...
//--------------------------------------------------------------------------------------------------
		...
		//---------------------------------------------------------------	
			...
			$pdf->rect(5,$fila,205,4,F);
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(35,4,utf8_decode('ANTECEDENTES '.$fech_crn),0,0,L);	
...
//			$texto12='En el siglo XVII, la sencillez y elegancia con que Isaac Newton había logrado explicar las leyes que rigen el movimiento de los cuerpos y el de los astros, unificando la física terrestre y la celeste, deslumbró hasta tal punto a sus contemporáneos que llegó a considerarse completada la mecánica. A finales del siglo XIX, sin embargo, era ya insoslayable la relevancia de algunos fenómenos que la física clásica no podía explicar. Correspondió a Albert Einstein superar tales carencias con la creación de un nuevo paradigma: la teoría de la relatividad, punto de partida de la física moderna.';
			...
			while($fil002=mysql_fetch_array($res002))		
			{
				...
				$optmamografia=$fil002['optmamografia'];
			}
			
//			$dbgeneropas=2;
			
			if($meno_crn=='S')$meno_crn1='SI';
			else $meno_crn1='NO';
			...
			if($dbgeneropas==2){
				...
				if($filasfinal>255)
				{
					...
					if($optmamografia=='S'){
						$fechaMamografia1_crn=$fechaMamografia_crn;	
					}
					else if($optmamografia=='N'){
						$fechaMamografia1_crn='No';	
					}
					else if($optmamografia=='A'){
						$fechaMamografia1_crn='No aplica';
					}
					else if($optmamografia==''){
						$fechaMamografia1_crn=$fechaMamografia_crn;
					}
					...
					$fila=$fila+4;
					$pdf->SetXY(5,$fila);
					$pdf->Cell(12,4,'FUM: '.utf8_decode($fechaFusajus).'             Menopausia: '.utf8_decode($meno_crn1) ,0,0,L);
					
					$fila=$fila+4;
					$pdf->SetXY(5,$fila);
					$pdf->Cell(12,4,'Menarquia: '.utf8_decode($mequcrm).'          Fecha de Citologia '.utf8_decode($fcito_crn). '                  Fecha de Mamografia '.$fechaMamografia_crn ,0,0,L);
...
//------------------------------------- Cambio 2024 ----------------------------------	
//vich		
if($feca_cpl<'2024-05-08'){			
// ESTE TOCA HABILITAR PORQUE ES EL TEST DE MORISKI VIEJO			
			...
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY(5,$fila);
			$pdf->Cell(90,4,utf8_decode(" ¿Se olvida de tomar alguna vez la medicacion?")  ,1,0,'L');
			$pdf->SetXY(95,$fila);
			$pdf->Cell(8,4,utf8_decode($testmori6)  ,1,0,'L');
			
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(90,4,utf8_decode(" ¿toma la medicacion a la hora indicada?")  ,1,0,'L');
			$pdf->SetXY(95,$fila);
			$pdf->Cell(8,4,utf8_decode($testmori1)  ,1,0,'L');
			...
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY(5,$fila);
			$pdf->Cell(90,4,utf8_decode(" Cuando se encuentra bien ¿deja de tomar su medicacion?")  ,1,0,'L');
			$pdf->SetXY(95,$fila);
			$pdf->Cell(8,4,utf8_decode($testmori2)  ,1,0,'L');
			
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(90,4,utf8_decode(" Si alguna vez se siente mal ¿deja de tomar su medicacion?")  ,1,0,'L');
			$pdf->SetXY(95,$fila);
			$pdf->Cell(8,4,utf8_decode($testmori3)  ,1,0,'L');
			...
			*/
			...
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->MultiCell(205,4, utf8_decode("Adherencia: ". $y08) ,0,L,0);		
			$fila=$pdf->GetY();
			$fila=$fila+1;
...
//--------------------------------------------------------------------------------------			
	...
	//FACTORES DE RIESGO
			...
			while($fil003=mysql_fetch_array($res003))		
			{
				...
				$tiempofumar_crn=$fil003['tiempofumar_crn'];
			}
			
//------------------------------------- Cambio 2024 --------------------------------------- 			
			$complicacioBlanda='';
			if($complicar_crn=='S'){$complicacioBlanda=$complicacioBlanda.'  Cadiacas SI       ';}
			else {$complicacioBlanda=$complicacioBlanda.' Cadiacas NO       ';}
			
			if($complicer_crn=='S'){$complicacioBlanda=$complicacioBlanda.' Cerebrales SI       ';}
			else {$complicacioBlanda=$complicacioBlanda.' Cerebrales NO       ';}
			
			if($compliret_crn=='S'){$complicacioBlanda=$complicacioBlanda.' Retinianas SI       ';}
			else {$complicacioBlanda=$complicacioBlanda.' Retinianas NO       ';}
			
			if($complivas_crn=='S'){$complicacioBlanda=$complicacioBlanda.' Vascular periferico SI       ';}
			else{$complicacioBlanda=$complicacioBlanda.' Vascular periferico NO       ';}
			
			if($compliren_crn=='S'){$complicacioBlanda=$complicacioBlanda.' Renales SI       ';}
			else {$complicacioBlanda=$complicacioBlanda.' Renales NO      ';}
			
			//$complicacioBlanda1=substr($complicacioBlanda, 0, -1);
			$complicacioBlanda1=$complicacioBlanda;
...
//coco			
			...
			if($fumaAbito=='SI'){	
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(60,4,utf8_decode("Numero de años de consumo:  ".$tiempofumar_crn )  ,0,0,'L');
				$fila=$fila+4;
				
				$indicePaquete=($nume_crn * $tiempofumar_crn)/20;
								
				$pdf->SetXY(5,$fila);
				$pdf->Cell(60,4,utf8_decode("Indice de paquetes año:  ".$indicePaquete )  ,0,0,'L');
				$fila=$fila+4;
			}
			...
			if($efec_crn1=='Ocasional' || $efec_crn1=='Mes' || $efec_crn1=='Quincenal' || $efec_crn1=='Semanal' || $efec_crn1=='Diario'){
				$pdf->SetXY(5,$fila);
			...
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("¿Se siente bien con su familia?  ".$tvio_des11 )  ,0,0,'L');
			$fila=$fila+4;
			
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("¿Cuando tiene problemas es apoyado por su familia?  ".$tvio_des12 )  ,0,0,'L');
			$fila=$fila+4;
		
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("¿En la familia existen expresiones de afecto y cariño?   ".$tvio_des13 )  ,0,0,'L');
			
			$fila=$fila+4;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("¿Se siente rechazado por alguien de su familia?   ".$tvio_des14 )  ,0,0,'L');
			
			$fila=$fila+4;
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("¿Ha sido agredido físicamente por alguien de su familia?  ".$tvio_des15 )  ,0,0,'L');
			
			$fila=$fila+4;
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("¿Ha tenido ideas o pensamientos suicidas?   ".$tvio_des16 )  ,0,0,'L');
	...
	//REVISION POR SISTEMAS
			...
			if($codrevis34=='S')$otroSintoma='SI'; else $otroSintoma='NO';
			...
			while($fil007=mysql_fetch_array($res007))		
			{
				...
				$otrorevisiste_rs=$fil007['otrorevisiste_rs'];
			}	
			...
			$pdf->Cell(60,4,utf8_decode(" Otros "),1,0,'L');
			$pdf->SetXY(65,$fila);
			$pdf->Cell(8,4,utf8_decode($otroSintoma)  ,1,0,'L');
			
			if($otroSintoma=='SI'){
				$fila=$fila+4;
				$pdf->SetXY(5,$fila);
				$pdf->MultiCell(205,4, utf8_decode("Cuales: ".$otrorevisiste_rs) ,0,L,0);		
				$fila=$pdf->GetY();
				$fila=$fila+1;
			}
		...
		//-----------------------------------------------------
			...
			$cad008="SELECT codf_crn, estf_crn, obsf_crn FROM cr_exfisico WHERE numhisto='$numhistohc'";
...
//-------------------------------------------------------------------
			...
			if($codf_crn1=='S')$codfinf_crn1='SI'; else $codfinf_crn1='NO'; 
			if($codf_crn2=='S')$codfinf_crn2='SI'; else $codfinf_crn2='NO';
			if($codf_crn3=='S')$codfinf_crn3='SI'; else $codfinf_crn3='NO';
			if($codf_crn4=='S')$codfinf_crn4='SI'; else $codfinf_crn4='NO';
			if($codf_crn5=='S')$codfinf_crn5='SI'; else $codfinf_crn5='NO';
			if($codf_crn6=='S')$codfinf_crn6='SI'; else $codfinf_crn6='NO';
			if($codf_crn7=='S')$codfinf_crn7='SI'; else $codfinf_crn7='NO';
			if($codf_crn8=='S')$codfinf_crn8='SI'; else $codfinf_crn8='NO';
			if($codf_crn9=='S')$codfinf_crn9='SI'; else $codfinf_crn9='NO';
			if($codf_crn10=='S')$codfinf_crn10='SI'; else $codfinf_crn10='NO';
			if($codf_crn11=='S')$codfinf_crn11='SI'; else $codfinf_crn11='NO';
			if($codf_crn12=='S')$codfinf_crn12='SI'; else $codfinf_crn12='NO';
			if($codf_crn13=='S')$codfinf_crn13='SI'; else $codfinf_crn13='NO';
			
//---------------------- Cambio 2024 ---------------------------			
			if($codf_crn14=='S')$codfinf_crn14='SI'; else $codfinf_crn14='NO';
//-------------------------------------------------------------------			
...
//-------------------- Cambio 2024 ----------------------------------			
			$pdf->SetXY(48,$fila);
			$pdf->MultiCell(161,4,utf8_decode($obsefis14),0,L,0);
			$fila62=$pdf->GetY();
			$pdf->SetXY(5,$fila);
			$pdf->Cell(28,4,utf8_decode(" Examen Mental")  ,0,0,'L');
			$pdf->SetXY(33,$fila);
			$pdf->Cell(15,4,utf8_decode($codfinf_crn14)  ,0,0,'L');
			$filamarco1=$fila62-$fila;	
			$pdf->Rect(5,$fila, 28, $filamarco1);
			$filamarco1=$fila62-$fila;	
			$pdf->Rect(33,$fila, 15, $filamarco1);
...
//-------------------------------------------------------------------			
	...
	//EXAMEN MENTAL
			...
			$cadExaMen="SELECT presenta, contacto, conciente, lenguaje, memoria, pensamiento, conversa, juicio, persepcion FROM cr_examenmental WHERE numhisto='$numhistohc'";
			$resExaMen=Mysql_query($cadExaMen);
			while($filExaMen=mysql_fetch_array($resExaMen))		
			{
				$presentaExmen=$filExaMen['presenta'];
				$contactoExmen=$filExaMen['contacto'];
				$concienteExmen=$filExaMen['conciente'];
				$lenguajeExmen=$filExaMen['lenguaje'];
				$memoriaExmen=$filExaMen['memoria'];
				$pensamientoExmen=$filExaMen['pensamiento'];
				$conversaExmen=$filExaMen['conversa'];
				$juicioExmen=$filExaMen['juicio'];
				$persepcionExmen=$filExaMen['persepcion'];
			}
			
			$cuentaExaMen=0;
			if($presentaExmen!='')$cuentaExaMen++;
			if($contactoExmen!='')$cuentaExaMen++;
			if($concienteExmen!='')$cuentaExaMen++;	
			if($lenguajeExmen!='')$cuentaExaMen++;
			if($memoriaExmen!='')$cuentaExaMen++;
			if($pensamientoExmen!='')$cuentaExaMen++;
			if($conversaExmen!='')$cuentaExaMen++;
			if($juicioExmen!='')$cuentaExaMen++;
			if($persepcionExmen!='')$cuentaExaMen++;
			...
			if($cuentaExaMen>0){
				...
				if($filasfinal>240)
				{
					...
					$pdf->SetFont('Arial','',9);
					$numpagtot1 = $numpagtot1+1;
					$pdf->Cell(128,4,$numpagtot1,0,0,'L');
					$fila=$fila+13;
				}
				...
				$pdf->SetFont('Arial','B',8);
				$pdf->SetXY(5,$fila);
				$pdf->Cell(204,4,utf8_decode("EXAMEN MENTAL")  ,1,0,'C');
				$pdf->SetFont('Arial','B',8);
				$fila=$fila+7;
				
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Examen Mental")  ,1,0,'C');
				
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode("Si")  ,1,0,'C');
				
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode("No"),1,0,'C');
				
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode("No Aplica"),1,0,'C');
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Buena presentacion personal")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($presentaExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($presentaExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($presentaExmen3),1,0,'C');
				$fila=$fila+4;
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Contacto visual y verbal esponaneo y atento")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($contactoExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($contactoExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($contactoExmen3),1,0,'C');
				$fila=$fila+4;
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Consciente, alerta y orientado en tiempo lugar y personas")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($concienteExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($concienteExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($concienteExmen3),1,0,'C');
				$fila=$fila+4;
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Lenguaje claro, comprensible, coherente y fluido")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($lenguajeExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($lenguajeExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($lenguajeExmen3),1,0,'C');
				$fila=$fila+4;
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Memoria conservada a corto y largo plazo")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($memoriaExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($memoriaExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($memoriaExmen3),1,0,'C');
				$fila=$fila+4;

				if($pensamientoExmen=='S'){
					$pensamientoExmen1='X';
					$pensamientoExmen2='';
					$pensamientoExmen3='';
				}
				if($pensamientoExmen=='N'){
					$pensamientoExmen1='';
					$pensamientoExmen2='X';
					$pensamientoExmen3='';
				}
				if($pensamientoExmen=='A'){
					$pensamientoExmen1='';
					$pensamientoExmen2='';
					$pensamientoExmen3='X';
				}
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Pensamiento logico, coherente")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($pensamientoExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($pensamientoExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($pensamientoExmen3),1,0,'C');
				$fila=$fila+4;
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Conversacion fluida con ideas logicas")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($conversaExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($conversaExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($conversaExmen3),1,0,'C');
				$fila=$fila+4;
				...
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("Juicio y raciocinio conservado")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($juicioExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($juicioExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($juicioExmen3),1,0,'C');
				$fila=$fila+4;

				if($persepcionExmen=='S'){
					$persepcionExmen1='X';
					$persepcionExmen2='';
					$persepcionExmen3='';
				}
				if($persepcionExmen=='N'){
					$persepcionExmen1='';
					$persepcionExmen2='X';
					$persepcionExmen3='';
				}
				if($persepcionExmen=='A'){
					$persepcionExmen1='';
					$persepcionExmen2='';
					$persepcionExmen3='X';
				}
				$pdf->SetXY(5,$fila);
				$pdf->Cell(100,4,utf8_decode("ensopercepcion sin alusinaciones ni ilusiones, sin ideas delirantes")  ,1,0,'L');
				$pdf->SetXY(105,$fila);
				$pdf->Cell(15,4,utf8_decode($persepcionExmen1)  ,1,0,'C');
				$pdf->SetXY(120,$fila);
				$pdf->Cell(15,4,utf8_decode($persepcionExmen2),1,0,'C');
				$pdf->SetXY(135,$fila);
				$pdf->Cell(17,4,utf8_decode($persepcionExmen3),1,0,'C');
			}
...
//------------------------------------------- Cambio 2024 --------------------------			
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY(5,$fila);
			$pdf->Cell(60,4,utf8_decode("COMPLICACIONES Y LESIONES EN ORGANO BLANCO:  ".$complicacioBlanda1 )  ,0,0,'L');
			$fila=$fila+4;
//----------------------------------------------------------------------------------------			
