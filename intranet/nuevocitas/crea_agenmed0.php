				...
				while($rs=mysql_fetch_array($bs))
				{
					...
					while($tursig<$turfin)
					{
						...
						if(mysql_num_rows($bhorcre)==0)
						{
							...
							$nomvar='check'.$i.'-'.$n;
							echo "<td width=40 align=center><input type=checkbox name=$nomvar value='1'><input type=button name=$nomvar class='boton4' value=$hora></td>";
						}
						else
						{
							...
							{							
								...
								{
									echo "<td width=40 align=center><input type=button name=$nomvar class='boton22' title='$nomare'  value=$hora></td>";
								}
								else
								{
									echo "<td width=40 align=center><input type=button name=$nomvar class='boton2' title='$nomare' onclick='cargamodal(\"$idhor\")' value=$hora></td>";
								}
							}
							else
							{							
								if($Usado_horario!=$ncita_horario || $numcit>0)
								{
									echo "<td width=40 align=center><input type=button name=$nomvar class='boton33' title='$nomare' value=$hora></td>";
								}
								else
								{
									echo "<td width=40 align=center><input type=button name=$nomvar class='boton3' title='$nomare' onclick='cargamodal(\"$idhor\")' value=$hora></td>";
								}
							}
      ...
      <div>
		...
		while($rhora=mysql_fetch_array($bhora))
		{
			...
			if($ncita_horario-$Usado_horario==0)
			{
				...
				ECHO "<BR><BR> SELECT areas_medic.areas_ar, areas.nom_areas, areas_medic.cod_ar
				FROM areas INNER JOIN areas_medic ON areas.cod_areas = areas_medic.areas_ar
				WHERE (((areas_medic.cod_med_ar)='$medico') AND ((areas.equi_area)='$areasel') AND ((areas_medic.esta_ar)='A')) <BR><BR>";
