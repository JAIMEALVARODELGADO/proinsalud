...
<body>
	...
	if($opc=='2')
	{
		...
		{		
			...
			if($docum != '')
			{
				...
				if(mysql_num_rows($bpac)>0)
				{
					while($rpac=mysql_fetch_array($bpac))
					{
						...
						$cad=" And citas_pendientes.paciente_pen='$codpac'";
			...
			WHERE (((citas_pendientes.fecha_pen)>='$fechaini' And (citas_pendientes.fecha_pen)<='$fechafin' $cad))
			...
			if($areasel != '')
			{
				...
				WHERE (((citas_pendientes.fecha_pen)>='$fechaini' And (citas_pendientes.fecha_pen)<='$fechafin') AND ((citas_pendientes.area_pen)='$areasel') $cad)
			...
			WHERE citas_pendientes.fecha_pen>='$fechaini' And citas_pendientes.fecha_pen<='$fechafin' $cad $consulta ORDER BY citas_pendientes.fecha_pen");
			

			$bpen=mysql_query("SELECT citas_pendientes.iden_pen, citas_pendientes.fecha_pen, citas_pendientes.paciente_pen, usuario.NROD_USU, Concat(usuario.PNOM_USU,' ',usuario.SNOM_USU,' ',usuario.PAPE_USU,' ',usuario.SAPE_USU) AS nombre, citas_pendientes.contrato_pen, contrato.NEPS_CON, citas_pendientes.medico_pen, medicos.nom_medi, citas_pendientes.area_pen, areas.nom_areas, citas_pendientes.tipo_pen, citas_pendientes.areatrabajo_pen, citas_pendientes.funcionario_pen, citas_pendientes.obse_pen
			FROM (((citas_pendientes LEFT JOIN usuario ON citas_pendientes.paciente_pen = usuario.CODI_USU) LEFT JOIN medicos ON citas_pendientes.medico_pen = medicos.cod_medi) INNER JOIN contrato ON citas_pendientes.contrato_pen = contrato.CODI_CON) INNER JOIN areas ON citas_pendientes.area_pen = areas.cod_areas 
			...
			*/
			...
			{						
				...
				WHERE (((citas.Idusu_citas)='$codusu') AND ((horarios.Fecha_horario)>='$fecha_pen') AND ((citas.Clase_citas)<'6'))");
