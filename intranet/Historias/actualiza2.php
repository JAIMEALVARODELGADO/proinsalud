...
<?php
//control de intervalo de fecha
	$fecha_actual=date('Y-m-d');
	$nuevafecha = strtotime ('-3 year' , strtotime($fecha_actual)); 
	$fecha_nueva_inicial = date ('Y-m-d',$nuevafecha);	
	$nuevafecha = strtotime ('+1 year' , strtotime($fecha_actual));
	$fecha_nueva_final = date ('Y-m-d',$nuevafecha);
	
	mysql_connect("localhost","root",""); 
	mysql_select_db("PROINSALUD");
	
//	$condicion="detalle_labs.estd_dlab<>'EL' ";
	$condicion="encabezado_labs.fchr_labs>='$fecha_nueva_inicial' AND encabezado_labs.fchr_labs<='$fecha_nueva_final' AND detalle_labs.estd_dlab<>'EL' ";
	if(!empty($usu))
	{
	...
	$sqlusu   = "SELECT usuario.CODI_USU, usuario.PNOM_USU, usuario.SNOM_USU, usuario.PAPE_USU, usuario.SAPE_USU, usuario.FNAC_USU, usuario.SEXO_USU, usuario.DIRE_USU, usuario.TRES_USU, contrato.NEPS_CON, ucontrato.ESTA_UCO
	...
	$resultusu=mysql_query($sqlusu);
	
	if(mysql_num_rows($resultusu) == 0)
	{
		echo "<h3 style='text-align:center;color:red;padding-top:10px;'>NO SE ENCONTRARON REPORTES DE LABORATORIO</h3>";
		exit();
	}
	...
	while($rowusu=mysql_fetch_array($resultusu))
	{
		...
		if($est_=='RE')
		{
			echo"<tr><td colspan=5><font size=2 face=arial color=red><b>NOTA: PACIENTE EN ESTADO RETIRADO</td></tr>";
		}
		
		$condicion="el.codi_usu='$iden_uco' AND dl.estd_dlab<>'EL'";
...
//			$condicion=$condicion.'AND dl.nord_dlab='.$n_ord;
		...
		$cons=mysql_query("SELECT el.iden_labs,dl.iden_dlab ,dl.estd_dlab,el.codi_usu,dl.codigo,dl.nord_dlab,el.ctr_labs,
		...
		while ($rowexa=mysql_fetch_array($cons))
		{
			...
			<td></td>
			<td></td>
			<td class='Td1' width=2%><a href='../uci/imprimir2_.php?codusu=$iden_uco&iden_labs=$iden_labs&nd_=$nord_dlab&band=1&ctr=$codi_con' target='fr01'><img src='icons/feed_magnify.png' border=0 width=17 height=17 alt='Imprimir' ></a>
