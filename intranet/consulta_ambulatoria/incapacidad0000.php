...
<script type="text/javascript">
	...
	(
		function() 
		{		
			$("#autonomcol").autocomplete("auto_establecimiento.php", {
			width: 600,
			minChars: 3,
			autoFill: false,
			mustMatch: false,
			...
			$("#automundepar").val(data['2']);
			$("#autodeparlab").val(data['3']);
			$("#automunilab").val(data['4']);
			});
		}	
	);		
...
<?
	...
	$bafi=mysql_query("SELECT usuario.SEXO_USU, usuario.TPAF_USU, cotadicional.SITU_COT, usuario.CODI_USU
	...
	if(file_exists($archivo2))
	{
		$fp = fopen ($archivo2, "r" );
		$reg1=0;
		while (( $data = fgetcsv ( $fp , 000 , "|" )) !== FALSE ) 
		{ 
			$reg1++;
			$j = 0;
	...
	if(file_exists($archivo2)  || $Gcod_mediconh=='98396211')
	{
		...
		/*if($tipoorden==1)
		{*/
		if($regmag_con=='S' && $tpafusu=='AC')echo"<input type=hidden name=tipoafi value='doc'>";
		else echo"<input type=hidden name=tipoafi value=''>";
		
		if($regmag_con=='S' && $tpafusu=='AC')
		{
		...
		<td align=center> <select class='caja' name=diagorden>
		<option value=''></option>
		...
		?>
		<script languaje='JAVASCRIPT'>uno.diagorden.value="<?echo $diagorden ?>";</script>
		<?php
		echo"
		<table align=center class='tbl' border=1 width=100%>
		<tr>
		...
		<td><textarea class='caja' name=obseinca class='caja' cols=100 rows=2>$obseinca</textarea></td>
		<td>
