...
<script language="JavaScript">
	...
	function ver()
	{		
		if(uno.ima.checked==true ||  uno.lab.checked==true || uno.rem.checked==true || uno.med.checked==true || uno.his.checked==true || uno.forpos.checked==true || uno.solqui.checked==true || uno.inca.checked==true || uno.reco.checked==true)
		{
	...
	if(!empty($codigousu))
	{
		if($tipo==1)
		{
			if($cedula!='')
			{
				...
				consultaprincipal.come_cpl, consultaprincipal.inca_cpl, consultaprincipal.vers_apli, consultaprincipal.cod_actpyp
				...
				WHERE (((encabesadohistoria.cous_ehi)=$codigousu))
				...
				while($rhi=mysql_fetch_array($bushisto))
				{
					...
					{	
						...
						echo"
						<tr>
						<td><a href='#' onclick='bushis(1, \"$nhisto\", \"$con\", \"$apli\",\"$codigousu\")'>$ticon - $nhisto</a></td>
			...
			{			
				...
				$bmed=mysql_query("select * from medicamentos_oncologia where num_histo='$numhisto'");
				if(mysql_num_rows($bmed)>0)$nummed='SI';
				...
				if(mysql_num_rows($bsq)>0)$numproc='SI';
					...
					$cadcro4="SELECT id_anes FROM anteced_aneste WHERE numcon_anes='$numhisto'";
						...
						if(!$resulcro4)echo 'no hay consulta';
