...
echo"<table align=center valign=top><tr><td>";
    ...
    //BUSCAR AUTORIZACIONES         
		...
		else
		{
			...
			echo"<tr>
			<th colspan=8>AUTORIZACIONES</th>				
			</tr>
			<tr>
			<th width=10%>SEL.</th>
		...
		WHERE (((permisos_citas.area_per)='$areatra') AND ((permisos_citas.usua_per)='$usucitas') AND ((permisos_citas.esta_per)='A') AND ((permisos_citascon.esta_pco)='A') AND ((permisos_citascon.cidi_pco)='A'))");
		...
		*/
		...
		{		
			if(($areas=='01' || $areas=='151') && $contrareal=='002' && ($munate=='52001' || $munate=='52356'))
			{
				...
				$bmedi="SELECT medicos.cod_medi, medicos.nom_medi FROM medicos WHERE $condicion";
		...
		{		
			if($areas=='01' && $contrareal=='002' && $munate=='52001')
			{
		...
		$bconce=mysql_query("select * from destipos where codt_des='94' order by nomb_des");
		echo "<td align=center><select class='caja' name=concesion>
		<option value='0'></option>";
		...
		if($areas=='80')
        {
			...
			//echo $areas.' '.$esprimera;
			if($esprimera=='S')
			{
				$chpri='';
				if($pricita=='S')$chpri='checked';
				echo"<tr>
				<th>PRIMERA CITA EN EL A&#209;O</th>
				<td>
					<input type=checkbox class='caja' $chpri name=pricita value='S'>
				</td>
				</tr>";
			}
	   ...
	   //if(!empty($medico))
		...
		<tr>
			<th><input type=button class='boton' id='botpen1' name=botpen value='Marcar como pendiente' onclick='pendientes(3)' style='display:none'></td>
		</tr>
		</table>
		</td></tr>
		</table>
...
if($proxi==1)
{
	...
	WHERE (((horarios.Fecha_horario)>='$hoy') AND ((horarios.Usado_horario)>'0') AND ((horarios.Cserv_horario)='$areas'))
