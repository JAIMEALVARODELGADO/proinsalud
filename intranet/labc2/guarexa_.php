...
<?
		...
		for($i=0;$i<$mcu;$i++)
		{
				...
				if($sel==1)
				{
					...
					$conex2=mysql_query("SELECT iden_dlab,codigo FROM detalle_labs WHERE codigo='$cod' AND iden_dlab='$iden_dlab'");
					
					if(mysql_num_rows($conex2)<>0)
					{
					  
					  mysql_query("Update detalle_labs SET cod_medi='$Gidusulab',obsv_dlab='$obs',refe_dlab='$ref',unid_dlab='$unlabc',estd_dlab='PR' WHERE iden_dlab='$iden_dlab'");
