...
<center><table class="Tbl0" border=1>	
	  <?php
            ...
            //Aqui se valida la informacion de medicamentos dispensados(formulacion) e insumos
            ...
            validamedicam($condicion,$iden_ctr,$codi_con,$nit_con);
             ...
             //Aqui se valida la informacion de insumos dispensados(formulacion)
            $condicion="contrato_for='$codi_con' AND (tido_for='1' Or tido_for='2' Or tido_for='6') AND cdis_for>0 AND (ISNULL(factu_for) Or factu_for<>'S') AND tdis_for<>'S' AND ";
            if(!empty($fechaini)){$condicion=$condicion."fdis_for between '".$fechaini."' AND ";}
            if(!empty($fechafin)){$condicion=$condicion."'".$fechafin."' AND ";}
            $condicion=substr($condicion,0,strlen($condicion)-5);
            $condicion=$condicion." ORDER BY nume_for";
            //echo "<br>".$condicion;
            validainsumos($condicion,$iden_ctr,$codi_con,$nit_con);
            ...
            //Aqui se valida la informacion de procedimientos medicos ambulatorios
            $condicion="codusu_proc<>0 AND contrato_proc='$codi_con' AND (ISNULL(factu_proc) OR factu_proc<>'S') AND ";
            if(!empty($fechaini)){$condicion=$condicion."(fecha_proc between '".$fechaini."' AND ";}
            if(!empty($fechafin)){$condicion=$condicion."'".$fechafin."') AND ";}
            $condicion=substr($condicion,0,strlen($condicion)-5);
            $condicion=$condicion." ORDER BY iden_proc";
            //echo "<br>".$condicion;            
            valida_procedimientos_medicos($condicion,$iden_ctr,$codi_con,$nit_con);
	  ?>
	</table>
        <?php
            if($errores!=0){
...
function validaconsultaext($condi_,$iden_ctr,$cont_,$nit_con,$fechaini_,$fechafin_){    
    ...
    WHERE $condi_";*/
    $sql_="SELECT iden_cpl,numc_cpl,id_cita,CODI_USU,NROD_USU,Cserv_horario AS area_cpl, nom_areas, codi_des, feca_cpl, cod1_cpl, coan_cpl,ccup_prim,ccup_cont,come_cpl FROM vista_consulta_externa
    WHERE $condi_";
    //echo $sql_;
    ...
    while($row=mysql_fetch_array($sql_)){        
        ...
        //echo "<br>".$GLOBALS['error_registro'];
        if($GLOBALS['error_registro']=='N'){            
            guardaregistro($row[CODI_USU],$cont_,$iden_ctr,$row[cod1_cpl],$codi_des,$nit_con,$row[iden_cpl],'consultaprincipal','iden_cpl','factu_cpl',$tipo_dfa,$iden_tco,$desc_map,1,$valo_tco,$row['come_cpl']);
        }
    }
}
    ...
    if(mysql_num_rows($conslab_)<>0){        
        while($row_=mysql_fetch_array($conslab_)){
            ...
            if(mysql_num_rows($constco)<>0){
                ...
                //Este proceso es temporal hasta que en laboratorio se capturen los codigos del los nuevos cups
                //echo "<br>AAAAAAquiiiiii ".$row_[codigo].' '.$row_[codi_cup];
                $consultahomo="SELECT codigo,codi_cup,esta_cup FROM cups WHERE codi_cup='$row_[codi_cup]' AND esta_cup='AC'";
                //echo "<br>".$consultahomo;
                $consultahomo=mysql_query($consultahomo);
                if(mysql_num_rows($consultahomo)>0){
                    $rowhomo=mysql_fetch_array($consultahomo);
                    $constco="SELECT codigo,iden_map,iden_map,iden_tco,iden_ctr,valo_tco,esta_tco FROM vista_tarco WHERE codigo='$rowhomo[codigo]' AND iden_ctr='$iden_ctr'";
                    //echo "<br>".$constco;
                    $constco=mysql_query($constco);
                    //echo "<br>".mysql_num_rows($constco);
                    if(mysql_num_rows($constco)<>0){
...
function validamedicam($condi_,$iden_ctr,$cont_,$nit_con){
    ...
    /*$consmed_="SELECT nume_for,coduni_usu,servicio_for,dxprin_for,regi_for,fdis_for,codi_pro,ncsi_medi,nomb_mdi,cdis_for 
FROM formedica.vista_formuladet 
WHERE contrato_for='135' AND (tido_for='1' Or tido_for='2' Or tido_for='6') AND cdis_for>0 AND (ISNULL(factu_for) Or factu_for<>'S') AND tdis_for<>'S' AND fdis_for between '2018-10-31' AND '2018-10-31' 
ORDER BY nume_for";*/
    //echo "<br>".$consmed_;
    ...
    if(mysql_num_rows($consmed_)<>0){
        while($row=mysql_fetch_array($consmed_)){            
            ...
            validamedctr($codi_,$iden_ctr,$iden_tco,$desc_map,$valo_tco,$tipo_dfa,$row[codi_pro],$row[nomb_mdi]);
            if($valo_tco<>0){
...
function validainsumos($condi_,$iden_ctr,$cont_,$nit_con){
    $consins_="SELECT nume_for,coduni_usu,servicio_for,dxprin_for,regi_for,fdis_for,codi_ins,desc_ins,cdis_for
    FROM formedica.vista_formula_insumos
    WHERE $condi_";
    //echo "<br>".$consins_;
    $consins_=mysql_query($consins_);
    if(mysql_num_rows($consins_)<>0){
        while($row=mysql_fetch_array($consins_)){            
            $codi_=$row[codi_ins];
            $iden_tco='';
            $desc_map='';
            $tipo_dfa='';
            $valo_tco=0;
            validamedctr($codi_,$iden_ctr,$iden_tco,$desc_map,$valo_tco,$tipo_dfa,$row[codi_ins],$row[desc_ins]);
            if($valo_tco<>0){
                guardaregistro($row[coduni_usu],$cont_,$iden_ctr,$row[dxprin_for],$row[servicio_for],$nit_con,$row['regi_for'],'formedica.formuladet','regi_for','factu_for',$tipo_dfa,$iden_tco,$desc_map,$row[cdis_for],$valo_tco,'');
            }
        }
...
function validarips_procedim($condi_,$iden_ctr,$cont_,$nit_con){            
    ...
    while($row=mysql_fetch_array($sql_)){
        ...
        if($area==''){
            $error_="Servicio ".$row[area_ecr]." NO homologado o servicio vacío en procedimientos";
            muestraerror($error_);
            $GLOBALS['errores']++;
            $error_reg_='S';
        }
...
function valida_procedimientos_medicos($condi_,$iden_ctr,$cont_,$nit_con){            
    $tipo_dfa="P";
    $cantidad='1';
    $sql_="SELECT iden_proc,codusu_proc, nrod_usu,dxingreso_proc,codproced_proc,codi_cup,area_proc,codi_des,medico_proc, fecha_proc
    FROM vista_procedimientos_medicos
    WHERE $condi_";
    //echo "<br><br>".$sql_;
    $sql_=mysql_query($sql_);
    $GLOBALS['nfacturas']=$GLOBALS['nfacturas']+mysql_num_rows($sql_);
    ...
    while($row=mysql_fetch_array($sql_)){
        ...
        $error_reg_='N';
        $cantidad=1;
        $cod_cie10=STRTOUPPER($row[dxingreso_proc]);
        $area=$row[codi_des];
        $constco="SELECT codigo,codi_cup,iden_map,iden_map,iden_tco,iden_ctr,descrip,valo_tco,esta_tco FROM vista_tarco WHERE codigo='$row[codproced_proc]' AND iden_ctr='$iden_ctr'";
        //echo "<br>".$constco;
        $constco=mysql_query($constco);
        //echo "<br>".mysql_num_rows($constco);
        if(mysql_num_rows($constco)<>0){
        ...
        if($area==''){
            $error_="Servicio ".$row[codi_des]." NO homologado o servicio vacío en procedimientos medicos";
            muestraerror($error_);
            $GLOBALS['errores']++;
            $error_reg_='S';
        }
        if($error_reg_=='N'){
            guardaregistro($row[codusu_proc],$cont_,$iden_ctr,$cod_cie10,$area,$nit_con,$row[iden_proc],'procedimientos_medicos','iden_proc','factu_proc',$tipo_dfa,$rowtco[iden_tco],$rowtco[descrip],$cantidad,$valor,$row[medico_proc]);
        }
    }
}
