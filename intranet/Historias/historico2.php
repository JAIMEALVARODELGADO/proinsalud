...
else //HISTORIA DE EVOLUCION
{
		$entra=0;
		$busing="SELECT ingreso_hospitalario.id_ing, ingreso_hospitalario.fecin_ing
		FROM ingreso_hospitalario
		WHERE (((ingreso_hospitalario.codius_ing)='$Gideusu')) ORDER BY ingreso_hospitalario.id_ing DESC";
		//echo $busing;
		...
		if(mysql_num_rows($busing)>0)
		{
			...
			while($row18=mysql_fetch_array($busing))
			{
				...
				<td align=center><a href='../uci/imprimir2_.php?codusu=$Gideusu&ingreso=$ingre' target='nuevo'><img src='img/682273389326331766.png' border=0 title='Laboratorios' width='30' hight='30'></a></td>
				<td align=center><a href='terapia_impre1.php?ingreso=$ingre&cedula=$cedula' target='nuevo'><img src='img/1292362969.png' border=0 title='Terapias' width='30' hight='30'></a></td>
				...
				//Aqui consulto si hay anexo técnico 9 diligenciado
				$sql_at9="select refer_at910.iden_rat,refer_at910.fecha_rat 
				from refer_at910
				inner join (hist_evo
				...
				where refer_at910.servorig_rat ='HO' and ingreso_hospitalario.id_ing ='$ingre'";
				//echo "<br>".$sql_at9;				
				$res_at9=mysql_query($sql_at9);
				if(mysql_num_rows($res_at9)>0){
					$at9= "<a href='lista_at9.php?cedula=$cedula&id_ing=$ingre' target='_self'><img src='../uci/img/AT9.ico' hight='25' width='25' border=0 title='Anexo Técnico 9'></a>";					
				}
				else{
					$at9="<a href='#'><img src='uci/img/AT9.ico' hight='25' width='25' border=0 title='Anexo Técnico 9'></a>";
				}
				echo "				
				<td>$at9</td>
				<td>$formula</td>
			...
			if(!empty($ingreso))
			{
				...
				WHERE (((ingreso_hospitalario.id_ing)='$ingreso')) ORDER BY ingreso_hospitalario.id_ing DESC");	
