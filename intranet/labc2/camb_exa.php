...
.Estilo7 {font-family: Arial, Helvetica, sans-serif; font-size: 9px; color: #333366; }
-->
</style>
...
function guarem(valor,otro,nrdl)
{
        ref="form1.iden_dlab"+otro+".value"; 
        eval(ref)
        form1.idel_.value=eval(ref);
        ref="form1.codigo"+otro+".value"; 
        eval(ref)
        ref="form1.ano"+valor+otro+".value"; 
        eval(ref)
        form1.ano_.value=eval(ref);
        ref="form1.mes"+valor+otro+".value";
        eval(ref)
        form1.mes_.value=eval(ref)
        ref="form1.dia"+valor+otro+".value";
        eval(ref)
        form1.dia_.value=eval(ref)
        ref="form1.labo"+valor+otro+".value";
        eval(ref)
        form1.labo_.value=eval(ref)
        
        form1.action='guaremtdo.php';
        form1.target='';
        form1.submit();
}
function activa(cont_)
{
  //alert('toy');
  var cmd="";
  cmd="form1.chkpro"+cont_+".checked";
  if(eval(cmd)==true)
  {
	cmd="form1.res_annar"+cont_+".disabled=false";
	eval(cmd);
	cmd="form1.btn11"+cont_+".disabled=false";
    eval(cmd);
  }
}
function gua_annar(op,id)
{
	//alert(op);
	//alert(id);
	form1.anar.value=id;
	form1.evaluanar.value='1';
	form1.target='';
	form1.action='guarda_annar.php';
	form1.submit();
...
<?
	...
	/*$conusu="SELECT NROD_USU,CODI_CON,CODI_USU, PNOM_USU, SNOM_USU, PAPE_USU, SAPE_USU, FNAC_USU, SEXO_USU, TPAF_USU,CONT_UCO,NEPS_CON,IDEN_UCO,CUSU_UCO 
		FROM usuario
		INNER JOIN ucontrato ON CODI_USU=CUSU_UCO
		INNER JOIN contrato ON CONT_UCO=CODI_CON
		WHERE  CUSU_UCO ='$iden_uco'";*/
	$conusu="SELECT usuario.CODI_USU, usuario.NROD_USU, contrato.CODI_CON, contrato.NEPS_CON, usuario.PNOM_USU, usuario.SNOM_USU, usuario.PAPE_USU, usuario.SAPE_USU, usuario.FNAC_USU, usuario.SEXO_USU, usuario.TPAF_USU, ucontrato.IDEN_UCO
	...
	if($opcex=='21')
	{
		...
		echo "<br><tr class='Td4' >
		<td class='Th0'><strong><center>OPCION</strong></td>
		<td class='Th0'><strong><center>Codigo</strong></td>
		...
		$conexa="SELECT dl.iden_dlab ,dl.iden_labs,dl.estd_dlab,dl.codigo,dl.nord_dlab,cp.descrip,
			dl.obsv_dlab, dl.refe_dlab,dl.unid_dlab,dl.cod_medi  
		    FROM detalle_labs AS dl
			INNER JOIN cups AS cp ON cp.codigo=dl.codigo
		    WHERE dl.iden_labs='$iden_labs' AND dl.estd_dlab<>'EL' AND dl.estd_dlab<>'P'";
		//echo $conexa;
		$conexa=mysql_query($conexa);
		$jl=0;
		while($rowex = mysql_fetch_array($conexa))  
		{
		   ...
		   echo "<input type=hidden name=$nomvar value=$iden_dlab>";	
			...
			//echo"<input type=hidden name=$nomvar value=$estd_dlab>";
                            ...
                            echo "<td><br><span class='Estilo7'>$descrp</span></td>";
                             $i=0;
                            if($estd_dlab=='RE')
                            {
                                ...
                                {
                                    //$conc='20'.$k;
                                    echo "<option value='$k'>20$k</option>"; 
                                }
                                $nomvar='mes'.$i.$jl;
                                echo"</select>                                               
                                Mes: <select name=$nomvar>
                                <option value=''></option>"; 
                                for($j=1;$j<13;$j++)
                                {
                                ...
                                echo"</select>";
                                $nomvar='obs'.$i.$jl;
                                echo"<td colspan=2>Labo:";
                                $labor=mysql_query("SELECT codi_des,codt_des,nomb_des FROM destipos  WHERE codt_des='97'");
                                $nomvar='labo'.$i.$jl;
                                echo"<select name= $nomvar>";
                                echo"<option value=''></option>"; 
                                while($rowlab=  mysql_fetch_array($labor))
                                {
                                    echo"<option value='$rowlab[codi_des]'>$rowlab[nomb_des]</option>"; 
                                }
                                echo"</select>";
                                echo"<input type=button name='gua' value=' G ' onclick='guarem($i,$jl,$iden_labs)'>";
                                echo"</td><br><br>";
...
//								echo "<td></td>";		
                           ...
                           echo" <br><td colspan=6><input type=button value=Guardar onClick='gua_bd(21,$jl)'></tr>";   
	...
	echo"<table class='Tbl0' width=80% >
	<tr>
    <td class='Td1' align='center'>Examenes de AnnarLab</td>
    </tr></TABLE>";
	
	/*echo "select id_inter_winsislab,COD_ANALITO,NOM_ANALITO,RESULTADO ,VR_MINIMO,VR_MAXIMO,UNIDADES 
	FROM labo_inter_winsislab  WHERE iden_labs=$iden_labs and RESULTADO<>' '";*/
	$consannar="select id_inter_winsislab,COD_ANALITO,NOM_ANALITO,RESULTADO ,VR_MINIMO,VR_MAXIMO,UNIDADES 
	FROM labo_inter_winsislab  WHERE iden_labs=$iden_labs and RESULTADO<>' '";
	//echo $consannar;
	$consannar=mysql_query($consannar);
	echo"<br><br><table width=80% height=20 border=0 align=left cellspacing=1 bordercolor=#BED1DB bgcolor=#FFFFFF>
		<tr><div align=center><th height=20 colspan=7 bgcolor=#BED1DB><span class=Estilo32>Caracteres Generales</span></th></div>
		</span></th></div></tr>";
	
	echo"<tr class=Estilo33><td width=35% bgcolor=#BED1DB colspan=2><div align=left><span>NOMBRE</span></div></td>";
	echo"<td width=15% bgcolor=#BED1DB><div align=center class=Estilo33><span >RESULTADO</span></div></td>";
	echo"<td width=10% bgcolor=#BED1DB><div align=center class=Estilo33><span >VL MINIMO</span></div></td>";
	echo"<td width=15% bgcolor=#BED1DB><div align=center class=Estilo33><span >VL MAXIMO</span></div></td>";
	echo"<td width=25% bgcolor=#BED1DB><div align=center class=Estilo33><span >UNIDADES</span></div></td>";
	echo"<td width=5% bgcolor=#BED1DB><div align=center class=Estilo33><span >OPCION</span></div></td>";
	echo"</tr>";	
	echo"<tr>";	
	$anar=0;
	while($rowannar = mysql_fetch_array($consannar))  
	{
		$nom_annar=$rowannar[NOM_ANALITO];
		$id_wlab=$rowannar[id_inter_winsislab];
		$res_annar=$rowannar[RESULTADO];
		$vlmin_annar=$rowannar[VR_MINIMO];
		$vlmax_annar=$rowannar[VR_MAXIMO];
		$unid_annar=$rowannar[UNIDADES];
		
				
		$nom_var='chkpro'.$anar;
		echo "<td><input type='checkbox' name='$nom_var' onclick='activa($anar)'></td>";
		
		$nom_var='id_wlab'.$anar;
		echo "<input type=hidden name='$nom_var' value='$id_wlab'>";
		
		echo"<td><input type=hidden name='nom_annar' value=$nom_annar>$nom_annar</td>";
		
		$nom_var='res_annar'.$anar;
		echo"<td><input type=text name=$nom_var  value='$res_annar' disabled></td>";
		
		echo"<td><input type=hidden name=vlmin_annar  value='$vlmin_annar' maxlength='12'>$vlmin_annar</td>
			<td><input type=hidden name=vlmax_annar  value='$vlmax_annar' maxlength='12'>$vlmax_annar</td>
			<td><input type=hidden name=unid_annar  value='$unid_annar' maxlength='12'>$unid_annar</td>";
		
		$nom_var='btn11'.$anar;
        echo "<td><input type=button name='$nom_var' value='GRD' disabled onclick='gua_annar(1,$anar)'>";
	...
	echo"<input type=hidden name='evaluanar'>";
	echo"<input type=hidden name='result'>";
	echo"<input type=hidden name='ideanar'>";
	echo"<input type=hidden name='anar' value=$anar>";
	echo "</table>";
	...
	echo "<tr>
        ...
        <td><strong><div align=left>Dilucin</div></td>
        </tr>";
	...
	{	
		...
		if($cod_loex=='904105')
		{
			...
			echo"<table class='Tbl0'>
			<tr><td class='Td1' align='center'><STRONG>HORMONA FOLCULO ESTIMULANTE - FSH</strong></td></tr>
			</table>";
		...
		echo"
		   ...
		   <tr><td>1 Ao </td><td>0.12 - 0.21 </td></tr>
		   <tr><td>1 - 6 Aos</td><td>0.03 - 0.32</td></tr>
		   <tr><td>7 - 12 Aos</td><td>0.03 - 0.68</td></tr>
		   <tr><td>13 - 17 Aos</td><td>0.28 - 11.1</td></tr>";
		...
		if($cod_loex=='906446')
		{
			...
			echo"<table class='Tbl0'>
		<tr><td class='Td1' align='center'><STRONG>ANTGENOS IgE</strong></td></tr>
		</table>";
		...
		echo"
		   ...
		   <tr><td>1 Ao </td><td>Hasta 15</td></tr>
		   <tr><td>1 - 5 Aos</td><td>Hasta 60</td></tr>
		   <tr><td>6 - 9 Aos</td><td>Hasta 90</td></tr>
		   <tr><td>10 - 15 Aos</td><td>Hasta 200</td></tr>
		...
		if($cod_loex=='902219')
		{
			...
			echo "</select></td>";
			echo "<td>Reaccin Leucocitaria</td>";
			echo"</tr>";
...

{
	...
	if($oplb2==3)
	{
		...
		if(mysql_num_rows($cons_cpr)<>0)
		{
			...
			echo "<td align='center'><input type=checkbox name='trz_amb' $var2></td>";
			...
			echo "<tr><td><input type=checkbox name='qui_exna' value='1' $var5></td>";
			echo "<td colspan=3>Quistes de Endolimax Nana</td>";
			echo "<td align='center'><input type=checkbox name='trz_gins'  $var6></td>";
...
echo "<input type=hidden name=ano_>";
echo "<input type=hidden name=mes_>"; 
echo "<input type=hidden name=dia_>"; 
echo "<input type=hidden name=labo_>";
echo "<input type=hidden name=idel_>";
