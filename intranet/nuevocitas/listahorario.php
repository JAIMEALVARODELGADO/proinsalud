<?     
    ...
    {
        /*
		$cadmed=mysql_query("SELECT areas.nom_areas, medicos.cod_medi, medicos.nom_medi, horarios.Cserv_horario, horarios.Fecha_horario, horarios.Hora_horario, horarios.ID_horario, grup_area.codi_grar, horarios.Usado_horario
        FROM ((horarios INNER JOIN medicos ON horarios.Cmed_horario = medicos.cod_medi) INNER JOIN areas ON horarios.Cserv_horario = areas.cod_areas) INNER JOIN grup_area ON areas.cod_areas = grup_area.coar_grar
        WHERE (((horarios.Fecha_horario)>='$fechaini' And (horarios.Fecha_horario)<='$fechafin') AND ((horarios.Hora_horario)>='$horai' And (horarios.Hora_horario)<='$horaf') AND ((grup_area.cogr_grar)='$grupo'))
        ORDER BY areas.nom_areas, medicos.nom_medi, horarios.Fecha_horario, horarios.Hora_horario"); 
		*/
		
		$cadmed=mysql_query("SELECT areas_1.nom_areas, areas.nom_areas as arereal, medicos.cod_medi, medicos.nom_medi, horarios.Cserv_horario, horarios.Fecha_horario, horarios.Hora_horario, horarios.ID_horario, grup_area.codi_grar, horarios.Usado_horario
		FROM ((horarios INNER JOIN medicos ON horarios.Cmed_horario = medicos.cod_medi) INNER JOIN areas ON horarios.Cserv_horario = areas.cod_areas) INNER JOIN (areas AS areas_1 INNER JOIN grup_area ON areas_1.equi_area = grup_area.coar_grar) ON areas.equi_area = areas_1.cod_areas
		WHERE (((horarios.Fecha_horario)>='$fechaini' And (horarios.Fecha_horario)<='$fechafin') AND ((horarios.Hora_horario)>='$horai' And (horarios.Hora_horario)<='$horaf') AND ((grup_area.cogr_grar)='$grupo'))
    ...
    if($impripor==2)
    {
        $cadmed=mysql_query("SELECT areas.nom_areas, medicos.cod_medi, medicos.nom_medi, horarios.Cserv_horario, horarios.Fecha_horario, horarios.Hora_horario, horarios.ID_horario, horarios.Usado_horario
        ...
        WHERE (((horarios.Cserv_horario)='$codarea') AND ((horarios.Fecha_horario)>='$fechaini' And (horarios.Fecha_horario)<='$fechafin') AND ((horarios.Hora_horario)>='$horai' And (horarios.Hora_horario)<='$horaf'))
        ORDER BY areas.nom_areas, medicos.nom_medi, horarios.Fecha_horario, horarios.Hora_horario");
    ...
    if($impripor==3)
    {
		...
		AND ((horarios.Hora_horario)>='$horai' And (horarios.Hora_horario)<='$horaf'))
    ...
    {           
        ...
        FROM contrato INNER JOIN ((usuario INNER JOIN citas ON usuario.CODI_USU = citas.Idusu_citas) 
		INNER JOIN ucontrato ON (citas.Cotra_citas = ucontrato.CONT_UCO) AND (usuario.CODI_USU = ucontrato.CUSU_UCO)) ON contrato.CODI_CON = ucontrato.CONT_UCO
        ...
        if($n%49==0 || $codmedico!=$cmed)
        {
            ...
            $ver=$ver+5;
            $pdf->SetXY($hor, $ver);
            $pdf->Cell(130,5,'MEDICO: '.$nommedico.'                                '.$codmedico, 0, 0,L);
    ...
    function cabeza(&$pdf1)
    {
        ...
        $pdf1->Text($hor1+80, $ver1+34, 'CITAS PROGRAMADAS');
        ...
        $pdf1->SetFont('Arial','',6);
        $pdf1->Text($hor1+168, $ver1+36, '22 de Diciembre de 2020');
