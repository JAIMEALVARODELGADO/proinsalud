<?PHP
	...
	//echo $paciente.' '.$numcita.' '.$Gcod_mediconh;
...
<script language="JavaScript">
	function valida(n)
	{
		...
		if(fcf=="")
		{
			uno.fcf.style.background= "orange";
			alert("La el valor del fcf. NO puede ser vacio");
			vrfca=false;
		}
		else{uno.fcf.style.background='#FFFFFF';}
		
		gluco=uno.gluco.value;
		if(gluco=="")
		{
			uno.gluco.style.background= "orange";
			alert("El valor de la glucometria. NO puede ser vacio");
			vrfca=false;
		}
		else{uno.gluco.style.background='#FFFFFF';}
...
<?PHP
	...
	echo"<tr>";
		...
		while($rowtpr = mysql_fetch_array($sql_tper))
		{
			...
			$cod=$rowtpr[codi_des];
			if($cod==$chk_tpr)echo"<input  type=Radio name=chk_tpr onclick='sumar()' checked id='chk_tpr' value=$rowtpr[codi_des]>   $rowtpr[nomb_des]&nbsp; &nbsp; &nbsp;&nbsp; &nbsp;";
			else echo"<input  type=Radio name=chk_tpr onclick='sumar()' id='chk_tpr' value=$rowtpr[codi_des]>   $rowtpr[nomb_des]&nbsp; &nbsp; &nbsp;&nbsp; &nbsp;";
		}
		echo "</td></tr>";
...
<th align=center width=10%><font face='verdana'><b>IMC</th>	
	...
	<th align=center width=10%><font face='verdana'><b>GLUCOMETRIA</th>
	</tr>
	
	<tr>	
	<td align=center width=10%><input type=text onPaste='return false' size=2 class='caja' id='peso' onblur='calculaimc()'  name=peso value='$peso' onKeyPress='return soloNumeros(event)' ></td>
	<td align=center width=10%><input type=text onPaste='return false' size=2 class='caja' id='talla' onblur='calculaimc()' name=talla value='$talla' onKeyPress='return soloNumeros(event)'></td>	
	...
	echo"<td align=center width=10%><input type=text onPaste='return false' size=2 class='caja' id='tenar1' name=tenar1 value='$tenar1' onKeyPress='return soloNumeros(event)'><input type=text onPaste='return false' size=2 class='caja' name=tenar2 value='$tenar2' onKeyPress='return soloNumeros(event)'></td>
	...
	else
	{
		...
		echo"<input type=hidden name=fcf value='N/A'>";
		$val=0;
	}
	
	
	
	echo"<td align=center width=10%><input type=text size=2 class='caja' name=gluco value='$gluco'></td>";
	echo"</tr></table>";
	/*///CATEGORIA
	...
	echo"<br><br><table align=center class='tbl' width=80% border=1>";
	echo"<tr><th colspan=10 align=center>Dolor</th></tr>
	</TABLE>
	
	<br><br>
	<table align=center background='img/formatos/escaladolor.png'  height=200 width='1357' border=0>	
	<tr>
	...
	echo"<table align=center class='tbl' width=1357 border=1>";
	
	
	$anc=905/11;
	$sql_dsc=mysql_query("SELECT codi_des,nomb_des, valo_des,val2_des  FROM destipos WHERE codt_des='A9' And valo_des='2' And codi_des>='A919' AND homo2_des='1' ORDER BY homo_esp");
	echo"<tr>";
	$n=0;
	while($rowdes = mysql_fetch_array($sql_dsc))
	{
		ECHO"<td align=center width=$anc>";
		$imagen=$rowdes[val2_des];
		echo"<br> <input  type=Radio name=chk_dlr  id='chk_dlr' value=$rowdes[codi_des]><br><br><font size='3'>$rowdes[nomb_des]</font>";
		echo"</td>";
		$n++;
	}
	...
	//abuso sexual
	echo"<br><br><table align=center class='tbl4' width=80% border=1>";
