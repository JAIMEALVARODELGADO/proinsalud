...
/*$consulta=("SELECT el.iden_evo,el.iden_labs,ih.id_ing,dl.estd_dlab,dl.nord_dlab
	FROM detalle_labs AS dl
	INNER JOIN encabezado_labs as el ON el.iden_labs=dl.iden_labs
	INNER JOIN hist_evo AS evo ON evo.iden_evo=el.iden_evo
	INNER JOIN ingreso_hospitalario AS ih ON ih.id_ing=evo.id_ing
	WHERE dl.estd_dlab<>'EL' AND ih.id_ing='$ingreso'
...
function imprelab($codusu,$iden_labs,$nd_,&$pdf){
	$fila=62;
	$col=20;
	$pdf->Image('imagenes\du.JPG',5,10,204,0,'','');
	$pdf->SetFont('Arial','',10);
	...
	$pdf->SetFont('Arial','',7);
	$pdf->SetXY(5,254);
	$pdf->Cell(40,5,'PR: Examen Preliminar - VL: Examen Validado    *: Examen Confirmado  - MDF. Examen Modificado' ,0);
	$nu=date("Y-m-d"); 
	$ho=strftime("%I:%M:%S");
	$cont=$cont+0;	
	...
	$inf=mysql_query("SELECT el.iden_labs, el.codi_usu, el.cod_medi, el.iden_evo, el.iden_cita, el.fchr_labs, el.fche_labs,el.ctr_labs,
	...
	WHERE el.codi_usu='$codusu' AND el.iden_labs='$iden_labs'");
	//echo $inf;
	...
	if ($bandera=="true")
	{
		...
		$sql_=Mysql_query("SELECT NEPS_CON  FROM  contrato WHERE CODI_CON='$ctr'");
		$rowcts=mysql_fetch_array($sql_);
		$pdf->SetXY(165,40);
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(40,5,$rowcts[NEPS_CON],0);
        ...
        $sqlwlab=Mysql_query("SELECT * FROM `labo_inter_winsislab` WHERE iden_labs='$iden_labs'");
        if(mysql_num_rows($sqlwlab)=='0')
        {
	//1. Examenes Varios listado
                    $result2=Mysql_query("SELECT detalle_labs.iden_dlab ,detalle_labs.iden_labs, detalle_labs.codigo, cups.codi_cup, cups.descrip, detalle_labs.obsv_dlab,detalle_labs.etdv_dlab,
                    ...
                    { 
                            ...
                            $consvrf=mysql_query("SELECT codigo,sexo_ire,coin_ire,edin_ire,unin_ire,cosp_ire,edsp_ire,unsp_ire,vlin_ire,vlsu_ire 
                                            ...
                                            if(($rowm[estd_dlab]=='CU') AND ($rowm[etdv_dlab]=='RE'))
                                    {
                                            /*//$pdf->SetFont('Arial','',8);
                                            $pdf->setxy(125,$fila);
                                            $pdf->cell(40,5,'Remitido',0,60);*/
	...
	//$cont=$cont+10;
		...
		/*
		$lSql="SELECT cups.codigo, labo_inter_winsislab.iden_labs, labo_inter_winsislab.iden_dlab, labo_inter_winsislab.cod_examen, 
         labo_inter_winsislab.NOM_ANALITO, labo_inter_winsislab.RESULTADO, labo_inter_winsislab.VR_MINIMO, labo_inter_winsislab.VR_MAXIMO, 
         labo_inter_winsislab.UNIDADES, labo_inter_winsislab.NUM_ORDEN, labo_inter_winsislab.iden_labs
         FROM labo_inter_winsislab INNER JOIN cups ON labo_inter_winsislab.COD_EXAMEN = cups.codigo
         WHERE  labo_inter_winsislab.NUM_ORDEN='$nd_' AND labo_inter_winsislab.iden_labs='$iden_labs' order by labo_inter_winsislab.cod_examen";
		 
		$leboc_file = fopen("LEBoC_2017.txt", "w");
		fputs($leboc_file, $lSql);
		fclose($leboc_file);
		*/
		
		
         $resultanna=mysql_query("SELECT cups.codigo, labo_inter_winsislab.iden_labs, labo_inter_winsislab.iden_dlab, labo_inter_winsislab.cod_examen, 
         labo_inter_winsislab.NOM_ANALITO, labo_inter_winsislab.RESULTADO, labo_inter_winsislab.VR_MINIMO, labo_inter_winsislab.VR_MAXIMO, 
         labo_inter_winsislab.UNIDADES, labo_inter_winsislab.NUM_ORDEN, labo_inter_winsislab.iden_labs
         FROM labo_inter_winsislab INNER JOIN cups ON labo_inter_winsislab.COD_EXAMEN = cups.codigo
         WHERE  labo_inter_winsislab.NUM_ORDEN='$nd_' AND labo_inter_winsislab.iden_labs='$iden_labs' order by labo_inter_winsislab.cod_examen");
        ...
        while($rowmana = mysql_fetch_array($resultanna))
        { 
                ...
                if ($bandera=="true")
                {
                        ...
                        $pdf->settextcolor(180,20,30);
                        $pdf->SetFont('Arial','',9);
                        $pdf->Cell(40,2,'Ordenannar/ '.$nd_,0);
                        $pdf->settextcolor(180,20,30);
                        $pdf->settextcolor(0,0,0);
		...
		if($fila>=190)
		{
			...
			$fila=30;
			$pdf->setxy(35,$fila);
			$pdf->cell(40,5,substr($rowmana["NOM_ANALITO"],0,40));
			
			//$pdf->SetFont('Arial','',8);
                         $res_val=$rowmana["RESULTADO"];
                         $pdf->setxy(125,$fila);
			$pdf->cell(40,5,$res_val,0,60);
                                
			$pdf->SetFont('Arial','',8);
			$pdf->setxy(170,$fila);
			$pdf->cell(40,5,$rowmana["VR_MINIMO"],0,60);
			
			
			$pdf->setxy(150,$fila);
			$pdf->cell(40,5,substr($rowmana["UNIDADES"],0,60));
                }
                else
                {
                        $fila=$fila+8;
			//$pdf->setxy(15,$fila);
			//$pdf->cell(40,5,substr($rowm["codigo"],0,60));
			...
			$pdf->SetFont('Arial','',8);
			$pdf->setxy(170,$fila);
			$pdf->cell(40,5,$rowmana["VR_MINIMO"].' '. $rowm["VR_MAXIMO"],0,60);
...
while($rowcdes=mysql_fetch_array($cdeshcg))
	{
			...
			$pdf->SetXY(65,$fila);
			$pdf->multiCell(145,5,$nomb_des,0,'J');
...
$fila=increm($fila,$pdf,$cod,$num_ord);
$pdf->SetXY(15,$fila);
$pdf->Cell(40,5,'23-40',0);


$pdf->SetXY(65,$fila);
$pdf->Cell(40,5,'2468-36142',0);*/
...
$pdf->Cell(40,5,"OTROS: ".$con,0);
	...
	`est_loex`, `obs_est`, `ige_loex`, `obs_ige`,`hgc_loex` ,
	`fnd_mcn`, `end_mcn`, `fni_mcn`, `eni_mcn`, `obs_mcn`,`khi_oex`,`khv_oex`,`tsh_oexa`,`esta_ord` 
	...
	if(mysql_num_rows($conotrex)<>0)
	{
		...
		{	
			...
			if($cod_loex=='904105')
			{
				...
				$fila=increm($fila+2,$pdf,$cod,$nd_);
				$pdf->SetXY(80,$fila);
				$pdf->Cell(40,5,"HORMONA FOLÍCULO ESTIMULANTE - FSH",0);
			...
			if($cod_loex=='904107')
			{
				...
				$fila=increm($fila+2,$pdf,$cod,$nd_);
				$pdf->SetXY(80,$fila);
				$pdf->Cell(40,5,"HORMONA LEUTERNIZANTE - LSH",0);
			...
			if($cod_loex=='904601')
			{
				...
				$fila=increm($fila,$pdf,$cod,$nd_);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'1 Año              0.12 - 0.21  ',0);

				$fila=increm($fila,$pdf,$cod,$nd_);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'1 - 6 Años       0.03 - 0.32 ',0);

				
				$fila=increm($fila,$pdf,$cod,$nd_);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'7 - 12 Años     0.03 - 0.68',0);

				//resultados
				$fila=increm($fila,$pdf,$cod,$nd_);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'13 - 17 Años   0.28 - 11.1 ',0);
			...
			if($cod_loex=='906446')
			{
				...
				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'1 Año                  Hasta 15',0);
				
				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'1 - 5 Años           Hasta 60',0);

				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'6 - 9 Años           Hasta 90',0);

				
				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'10 - 15 Años       Hasta 200',0);
		...
		if($cod_loex=='902219')
			...
			if($cod_loex=='904903')
			{
				...
				/*$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'MUESTRA DE CORDON UMBILICAL',0);
			...
			if($cod_loex=='906610')
			{
				...
				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila+2);
				$pdf->Cell(40,5,'Hombres 35 - 40 Años: 0.5 - 1.3 ng/ml',0);

				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila+2);
				$pdf->Cell(40,5,'Hombres 46 - 55 Años: 0.6 - 2.1 ng/ml',0);
				
				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila+2);
				$pdf->Cell(40,5,'Hombres 56 - 65 Años: 0.7 - 3.2 ng/ml',0);
							
				$fila=increm($fila,$pdf,$cod,$num_ord);
				$pdf->SetXY(15,$fila+2);
				$pdf->Cell(40,5,'Hombres 65 - 81 Años: 1.3 - 6.4 ng/ml',0);
	...
	if(mysql_num_rows($res_alc)<>0)
	{
		...
		while($rowalc= mysql_fetch_array($res_alc))
		{
			...
			if($num_mue=='1')
			{
				...
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nº 1  Positivo Para BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
				else
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nº 1  No se Observan BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
			...
			if($num_mue=='2')
			{
				...
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nº 2  Positivo Para BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
				else
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nº 2  No se Observan BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
			...
			if($num_mue=='3')
			{
				...
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nº 3  Positivo Para BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
				else
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nº 3  No se Observan BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
...
if(!empty($ingreso)){
	$consulta=Mysql_query("SELECT el.iden_evo,el.iden_labs,ih.id_ing,dl.estd_dlab,dl.nord_dlab
	FROM detalle_labs AS dl
	INNER JOIN encabezado_labs as el ON el.iden_labs=dl.iden_labs
	INNER JOIN hist_evo AS evo ON evo.iden_evo=el.iden_evo
	INNER JOIN ingreso_hospitalario AS ih ON ih.id_ing=evo.id_ing
	...
	if(mysql_num_rows($consulta)<>0){
		while($row=mysql_fetch_array($consulta)){
			...
			$nd_=$row[nord_dlab];
			$pdf->AddPage();
			imprelab($codusu,$iden_labs,$nd_,$pdf);
		}
	}
