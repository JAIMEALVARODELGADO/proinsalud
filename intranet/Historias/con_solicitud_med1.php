...
function eliminareg(reg,ing,can,pro)
{
	uno.registroeli.value=reg;
	uno.ingresoeli.value=ing;
	uno.cantidadeli.value=can;
	uno.productoeli.value=pro;
...
<?
	...
	$bpermi=mysql_query("SELECT * FROM `permisos_especiales` WHERE `cod_apli` = '17' AND `cod_permi` = '01' AND `usuario_permi` = '$Gcod_medico' AND `estado_permi` = 'A'");
	if(mysql_num_rows($bpermi)>0)$permi_elimina='S';
	...
	<input type=hidden name=registroeli>	
	<input type=hidden name=ingresoeli>
	<input type=hidden name=cantidadeli>
	<input type=hidden name=productoeli>
	<input type=hidden name=elimina>";
	if($elimina=='SI')
	{
		$bsaldo=mysql_query("SELECT * FROM `saldoinsumoxpac` WHERE `id_ing` = '$ingresoeli' AND `idin_sxp` = '$productoeli'");
		while($rsaldo=mysql_fetch_array($bsaldo))
		{
			$idensaldo=$rsaldo['iden_sxp'];
			$entrasaldo=$rsaldo['entr_sxp'];
			$salesaldo=$rsaldo['sali_sxp'];
		}
		$saldo=$entrasaldo-$salesaldo;
		$nsalida=$salesaldo-$cantidadeli;	
		$nsaldo=$entrasaldo-$nsalida;
		$eliadmin=mysql_query("DELETE FROM administra_insumo WHERE iden_adi='$registroeli'");
		if($eliadmin)
		{
			$modisal=mysql_query("UPDATE saldoinsumoxpac SET sali_sxp='$nsalida' WHERE iden_sxp='$idensaldo'");		
		}
	}
	echo"
	...
	{		
		...
		$ntab=mysql_query("INSERT INTO medsoltmp SELECT movi_med.iden_mme AS numreg, movi_med.fsol_mme AS fecsol, movi_med.hsol_mme AS horsol, 
		movi_med.usso_mme AS ususol, movi_med.nomb_usu AS codsol,
		movi_med.nomb_usu AS nomsol, movi_med.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, medicamentos2.codi_mdi AS codmed, CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, movi_med.ndso_mme AS canti, destipos.nomb_des AS servi
		...
		AND ((destipos.codt_des)='20') AND ((ingreso_hospitalario.id_ing)='$ingreso'))");
		if(!$ntab)echo "NO 0<BR>";
				
		//insumos de insumed
		$ntab1=mysql_query("INSERT INTO medsoltmp SELECT movi_ins.iden_mov AS numreg, movi_ins.fsol_mov  AS fecsol, movi_ins.hsol_mov AS horsol, movi_ins.emple_mov AS ususol, 
		movi_ins.nomb_usu AS codsol, movi_ins.nomb_usu AS nomsol, movi_ins.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, 
		...
		WHERE (((concat(movi_ins.fsol_mov,movi_ins.hsol_mov))>='$fechorini' And (concat(movi_ins.fsol_mov,movi_ins.hsol_mov))<='$fechorfin') AND ((destipos.codt_des)='20') AND ((ingreso_hospitalario.id_ing)='$ingreso'))");
		if(!$ntab1)echo "NO 1<BR>";
				
		//medicamentos de insumed
		$ntab2=mysql_query("insert into medsoltmp SELECT movi_med.iden_mov AS numreg, movi_ins.fsol_mov AS fecsol, movi_ins.hsol_mov AS horsol, movi_ins.emple_mov AS ususol, 
		movi_ins.nomb_usu AS codsol, movi_ins.nomb_usu AS nomsol, movi_ins.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, medicamentos2.codi_mdi AS codmed, CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, movi_ins.caso_mov AS canti, destipos.nomb_des AS servi
		...
		WHERE (((concat(movi_ins.fsol_mov,movi_ins.hsol_mov))>='$fechorini' And (concat(movi_ins.fsol_mov,movi_ins.hsol_mov))<='$fechorfin') AND ((destipos.codt_des)='20') AND ((ingreso_hospitalario.id_ing)='$ingreso'))");
		$cad="SOLICITUD";
		if(!$ntab2)echo "NO 2<BR>";
	}	
	if($opcion==2)	//DISPENSACION
	{		
		//medicamentos de movimed
		$ntab=mysql_query("insert into medsoltmp SELECT movi_med.iden_mme AS numreg, movi_med.fdis_mme AS fecsol, movi_med.hdis_mme AS horsol, movi_med.usdi_mme AS ususol, 
		movi_med.nudi_mme AS codsol, movi_med.nudi_mme AS nomsol, movi_med.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, medicamentos2.codi_mdi AS codmed, CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, movi_med.nddi_mme AS canti, destipos.nomb_des AS servi
		...
		WHERE (((concat(movi_med.fdis_mme,movi_med.hdis_mme))>='$fechorini' And (concat(movi_med.fdis_mme,movi_med.hdis_mme))<='$fechorfin') 
		AND ((destipos.codt_des)='20')  AND ((ingreso_hospitalario.id_ing)='$ingreso'))");
		if(!$ntab)echo "NO 0<BR>";
				
		
		//insumos de insumed
		$ntab1=mysql_query("insert into medsoltmp SELECT movi_ins.iden_mov AS numreg, movi_ins.fdis_mov AS fecsol, movi_ins.hodi_mov AS horsol, movi_ins.udis_mov AS ususol, 
		movi_ins.nudi_mov AS codsol, movi_ins.nudi_mov AS nomsol, movi_ins.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, insu_med.codi_ins AS codmed, insu_med.desc_ins AS prod, movi_ins.cdfa_mov AS canti, destipos.nomb_des AS servi
		FROM (((movi_ins INNER JOIN ingreso_hospitalario ON movi_ins.id_ing = ingreso_hospitalario.id_ing) INNER JOIN usuario ON ingreso_hospitalario.codius_ing = usuario.CODI_USU) INNER JOIN insu_med ON movi_ins.codi_ins = insu_med.codi_ins) LEFT JOIN destipos ON movi_ins.area_mov = destipos.valo_des
		WHERE (((concat(movi_ins.fdis_mov,movi_ins.hodi_mov))>='$fechorini' And (concat(movi_ins.fdis_mov,movi_ins.hodi_mov))<='$fechorfin') 
		AND ((destipos.codt_des)='20')  AND ((ingreso_hospitalario.id_ing)='$ingreso'))");
		if(!$ntab1)echo "NO 1<BR>";
		
		
		//medicamentos de insumed
		$ntab2=mysql_query("insert into medsoltmp SELECT movi_ins.iden_mov AS numreg, movi_ins.fdis_mov AS fecsol, movi_ins.hodi_mov AS horsol, movi_ins.udis_mov AS ususol, 
		movi_ins.nudi_mov AS codsol, movi_ins.nudi_mov AS nomsol, movi_ins.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, medicamentos2.codi_mdi AS codmed, CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, movi_ins.cdfa_mov AS canti, destipos.nomb_des AS servi
	...
	{
		$ntab=mysql_query("insert into medsoltmp SELECT recepcion_insu.iden_rec AS numreg, recepcion_insu.fech_rec AS fecsol, recepcion_insu.hora_rec AS horsol, recepcion_insu.usua_rec AS ususol, 
		recepcion_insu.nomb_usu AS codsol, recepcion_insu.nomb_usu AS nomsol, recepcion_insu.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, medicamentos2.codi_mdi AS codmed, CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, recepcion_insu.cant_rec AS canti, destipos.nomb_des AS servi
		FROM ((usuario INNER JOIN ((recepcion_insu INNER JOIN medicamentos2 ON recepcion_insu.idin_rec = medicamentos2.codi_mdi) 
		INNER JOIN ingreso_hospitalario ON recepcion_insu.id_ing = ingreso_hospitalario.id_ing) ON usuario.CODI_USU = ingreso_hospitalario.codius_ing) 
		INNER JOIN forma_farmaceutica ON medicamentos2.coff_mdi = forma_farmaceutica.codi_ffa) LEFT JOIN destipos ON recepcion_insu.area_rec = destipos.valo_des
		WHERE (((concat(recepcion_insu.fech_rec,recepcion_insu.hora_rec))>='$fechorini' And (concat(recepcion_insu.fech_rec,recepcion_insu.hora_rec))<='$fechorfin') AND ((recepcion_insu.tpin_rec)='M') AND ((destipos.codt_des)='20') AND ingreso_hospitalario.id_ing='$ingreso')");
		if(!$ntab)echo "NO 0<BR>";
		
				
		$ntab1=mysql_query("insert into medsoltmp SELECT recepcion_insu.iden_rec AS numreg, recepcion_insu.fech_rec AS fecsol, recepcion_insu.hora_rec AS horsol, recepcion_insu.usua_rec AS ususol, 
		recepcion_insu.nomb_usu AS codsol, recepcion_insu.nomb_usu AS nomsol, recepcion_insu.id_ing AS cedu, CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, insu_med.codi_ins AS codmed, insu_med.desc_ins AS prod, recepcion_insu.cant_rec AS canti, destipos.nomb_des AS servi
		...
		WHERE (((concat(recepcion_insu.fech_rec,recepcion_insu.hora_rec))>='$fechorini' And (concat(recepcion_insu.fech_rec,recepcion_insu.hora_rec))<='$fechorfin') AND ((recepcion_insu.tpin_rec)='I') AND ((destipos.codt_des)='20') AND ingreso_hospitalario.id_ing='$ingreso')");
		if(!$ntab1)echo "NO 1<BR>";
		$cad="RECEPCION";
	...
	{		
		...
		$ntab=mysql_query("insert into medsoltmp SELECT 
		administra_insumo.iden_adi AS numreg, 
		administra_insumo.fech_realadm AS fecsol, 
		administra_insumo.hora_realadm AS horsol, 
		administra_insumo.obse_adi AS ususol, 
		administra_insumo.resp_adi AS codsol, 
		administra_insumo.nomb_usu AS nomsol, 
		administra_insumo.id_ing AS cedu, 
		CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, 
		medicamentos2.codi_mdi AS codmed, 
		CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, 
		administra_insumo.cant_adi AS canti, 
		destipos_1.nomb_des AS servi
		FROM (((medicamentos2 
		INNER JOIN ((usuario 
		INNER JOIN ingreso_hospitalario ON usuario.CODI_USU = ingreso_hospitalario.codius_ing) 
		INNER JOIN administra_insumo ON ingreso_hospitalario.id_ing = administra_insumo.id_ing) ON medicamentos2.codi_mdi = administra_insumo.idin_adi) 
		INNER JOIN forma_farmaceutica ON medicamentos2.coff_mdi = forma_farmaceutica.codi_ffa) LEFT JOIN destipos ON administra_insumo.cama_adi = destipos.codi_des) LEFT JOIN destipos AS destipos_1 ON destipos.val2_des = destipos_1.codi_des
		WHERE (((concat(administra_insumo.fech_realadm,administra_insumo.hora_realadm))>='$fechorini' And (concat(administra_insumo.fech_realadm,administra_insumo.hora_realadm))<='$fechorfin') AND ((administra_insumo.tpin_adi)='M') AND ((administra_insumo.id_ing)='$ingreso'))
		GROUP BY numreg, fecsol, horsol, ususol, codsol, nomsol, cedu, nompac, codmed, prod, canti, servi");
		
		
		if(!$ntab)echo "NO 0<BR>";

		
		$ntab1=mysql_query("insert into medsoltmp SELECT 
		administra_insumo.iden_adi AS numreg, 
		administra_insumo.fech_realadm AS fecsol, 
		administra_insumo.hora_realadm AS horsol, 
		administra_insumo.obse_adi AS ususol,
		...
		administra_insumo.id_ing AS cedu, 
		CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, 
		insu_med.codi_ins AS codmed, 
		insu_med.desc_ins AS prod, 
		administra_insumo.cant_adi AS canti, 
		destipos_1.nomb_des AS servi
		...
		WHERE (((concat(administra_insumo.fech_realadm,administra_insumo.hora_realadm))>='$fechorini' And (concat(administra_insumo.fech_realadm,administra_insumo.hora_realadm))<='$fechorfin') AND ((administra_insumo.tpin_adi)='I') AND ((administra_insumo.id_ing)='$ingreso'))");
		if(!$ntab1)echo "NO HAY INSERT";
		$cad="ADMINISTRACION";
		if(!$ntab1)echo "NO 1<BR>";
	...
	{
		$sql="insert into medsoltmp SELECT 
		devol_insumos.iden_dev AS numreg, 
		devol_insumos.fech_dev AS fecsol, 
		devol_insumos.hora_dev AS horsol, 
		devol_insumos.resp_dev AS ususol, 
		devol_insumos.nomb_usu AS codsol,
		devol_insumos.nomb_usu AS nomsol, 
		devol_insumos.id_ing AS cedu, 
		CONCAT(usuario.PAPE_USU,' ',usuario.SAPE_USU,' ',usuario.PNOM_USU,' ',usuario.SNOM_USU) AS nompac, 
		medicamentos2.codi_mdi AS codmed, 
		CONCAT(medicamentos2.nomb_mdi,' ',medicamentos2.noco_mdi,' ',forma_farmaceutica.desc_ffa) AS prod, 
		devol_insumos.cant_dev AS canti, 
		destipos.nomb_des AS servi		
		...
		WHERE (((concat(devol_insumos.fech_realdev,devol_insumos.hora_realdev))>='$fechorini' And (concat(devol_insumos.fech_realdev,devol_insumos.hora_realdev))<='$fechorfin') AND ((devol_insumos.tipo_dev)='M') AND ((destipos.codt_des)='20') AND ingreso_hospitalario.id_ing='$ingreso')";
		$ntab=mysql_query($sql);
		if(!$ntab)echo $sql."<BR><BR>";
		
		
		$sql1="insert into medsoltmp SELECT 
		devol_insumos.iden_dev AS numreg, 
		devol_insumos.fech_dev AS fecsol, 
		devol_insumos.hora_dev AS horsol, 
		devol_insumos.resp_dev AS ususol,
		...
		WHERE (((concat(devol_insumos.fech_realdev,devol_insumos.hora_realdev))>='$fechorini' And (concat(devol_insumos.fech_realdev,devol_insumos.hora_realdev))<='$fechorfin') AND ((devol_insumos.tipo_dev)='I') AND ((destipos.codt_des)='20') AND ingreso_hospitalario.id_ing='$ingreso')";
		$ntab1=mysql_query($sql1);	
		if(!$ntab1)echo $sql1."<br><BR>";
		$cad="DEVOLUCION";
	}
	$bus=mysql_query("select * from medsoltmp");
		...
		$bpac=mysql_query("SELECT medsoltmp.cedu AS MxDecedu, medsoltmp.nompac
		FROM medsoltmp WHERE 1 $cadena3 GROUP BY medsoltmp.cedu, medsoltmp.nompac ORDER BY medsoltmp.nompac");
	...
	ECHO"
	<input type=hidden name=nominfo value='$nominfo'>
	<center><table align=center class='tbl' cellpadding=2 border=1 bgcolor='#E3F2F2'>
	<tr>
	<th>
	...
	if($tipoconsul==1) //detalle
	{
		...
		if(!empty($opcion))
		{
			if(mysql_fetch_array($bpro)>=0)
			{
				...
				<tr align=center>";
				if($opcion==4  && $permi_elimina=='S')
				{
					echo"<th align=center>ELIMINAR</th>";
				}				
				echo"<th><a href='#' onclick='orden(\"fecsol\")'>FECHA</a></td>
				...
				if($opcion==4 && $permi_elimina=='S')echo"<th><a href='#' onclick='orden(\"canti\")'>OBSERVACION</a></td>";
				//<th>ESTADO</td>	
				ECHO"<tr>
				<tr align=center>";
				if($opcion==4 && $permi_elimina=='S')
				...
				while($rpac=mysql_fetch_array($bpac))
				{
					...
					$nompac=$rpac['nompac'];
					if($codpac==$cedula)echo"<option value='$codpac' selected>$codpac $nompac</option>";
					else echo"<option value='$codpac'>$codpac $nompac</option>";
				}	
				echo"</select>	
				</td>	
				<td>
				...
				while($rpac=mysql_fetch_array($bpro))
				{
					...
					if($codpro==$producto)echo"<option selected value='$codpro'>$nompro</option>";
				...
				{
					$registro=$rtodo['numreg'];
					$fecbus=$rtodo['fecsol'];
					$horbus=$rtodo['horsol'];
					$codisol=$rtodo['codsol'];
					...
					$codproducto=strtoupper($rtodo['codmed']);
					...
					if($opcion==4  && $permi_elimina=='S')
					{
						echo"<td align=center><a href='#' onclick='eliminareg($registro,$ingcama,$canbus,\"$codproducto\")'>>></a></td>";
					}
					echo"<td align=center>$fecbus</td>
					...
					$nomvar='codisol'.$n;
					echo"<input type=hidden name=$nomvar value='$codisol'>";
					...
					$nomvar='obsbus'.$n;
					echo"<input type=hidden name=$nomvar value='$obser'>";	
					$n++;				
				}				
