...
<?
	...
	$fecha=date("Y-m-d");
	$fechaant=date("Y-m-d",strtotime($fecha."- 2 days"));
    $hora=date("H-i");
	...
	{				
		...
		WHERE (((triage_urgencias.dest_tri)='') AND ((citas.Esta_cita)<>'2') AND ((citas.Clase_citas)<='5') AND ((horarios.Cserv_horario)='$Gareanh') AND ((horarios.Fecha_horario)<='$fecha' And (horarios.Fecha_horario)>='$fechaant') AND ((horarios.Cmed_horario)='1101'))
	...
	{		
		...
		{			
			...
			$bcon=mysql_query("SELECT NEPS_CON FROM contrato WHERE CODI_CON='$contrato'");
			$rcon=mysql_fetch_array($bcon);
			$ncontra=$rcon['NEPS_CON'];
