...
<SCRIPT LANGUAGE="JavaScript">
	...
	function valida()
	{
		uno.action='factura_solicitud_g.php';
		uno.target='';
		uno.submit();
	}
	function CloseModal() 
	...
	{
		var URL="../facturacion/fac_2previo.php?iden_fac="+factu_; 
		var titulo="Factura" 
		var x=0 
		var y=0 
		var ancho=1000
...
<?	
	...
	<tr>
	<th colspan=10 valign=center align=center hwight=40>PACIENTES CITADOS</th>
	</tr>
	</table><br>";
	...
	FROM areas INNER JOIN municipio ON areas.muni_area = municipio.CODI_MUN
	...
	<div>
		...
		if($estadosel=='SO')echo"<input type=checkbox name=estadonuevo value='PE'> CANCELAR SOLICITUD DE FACTURACION";
		...
		if($estadosel!='FI')
		{
			if($estadosel=='FA')
			{
				...
				<td><input type='file' id='archivo[]' name='archivo[]' multiple='' accept='application/pdf, .jpg,, .jpeg, .bmp, .png'></td>
				</tr>";
			}
			echo"
			<tr>
		...
		while($rseg=mysql_fetch_array($bseg))
		{
			...
			if($estado=="FA")
			{
				$bfactu=mysql_query("SELECT encabezado_factura.iden_fac
				FROM encabezado_factura
				WHERE (((encabezado_factura.nume_fac)='$factu') AND ((encabezado_factura.pref_fac)='FE'))");
				$rfactu=mysql_fetch_array($bfactu);
				$iden_fac=$rfactu['iden_fac'];
				$nfactu=$factu;
			}
			...
			<td>$obser</td>
			<td>$usua</td>			
			<td class='Td2'><a href='#' onclick='imprimir($iden_fac)' title='Imprimir'>$nfactu</a></td>
			</tr>";
			$n++;
		}
