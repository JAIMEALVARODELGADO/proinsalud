		...
		$i=0;
			...
			for($n=0;$n<$fin2;$n++)
			{
				...
				if(!empty($idcitaasig))
				{
					...
					$mensacita="CITA ASIGNADA CON LABORATORIO PARA EL ".$fectex." A LAS ".$horcita;	
				}
				if($fecha_estimada!="0000-00-00")
				{
	...
	if($med=='1')
	{
		...
		if($nonc>0)
		{
			...
			{			
				...
				$pdf->Cell(165,5,$nompronco,1,0,L);
				$pdf->SetFont('Arial','',7);
...
$fila=$fila+$h;
		...
		while($rexaf=mysql_fetch_array($bexafis))
		{
			...
			if($talla!=''){$pdf->Cell(23,4,'Talla: '.$talla.' cm.',0,0,L);}
			if($pcfa!=''){$pdf->Cell(23,4,'P.C: '.$pcfa,0,0,L);}
			if($imc!=''){$pdf->Cell(23,4,'IMC: '.$imc.' '.$desimg,0,0,L);}
...
//---------------------------	
	...
	if($inca==1)
	{
			...
			if($tformato_inca=='D')
			{
				$fila=$fila+5;
				$pdf->SetXY(10,$fila);
				$pdf->Cell(20,5,'PLANTEL EDUCATIVO: ______________________________________________________________  MUNICIPIO: ____________________________',0,0,L);
				
				$pdf->SetXY(45,$fila);
				$pdf->Cell(115,5,$estedu_inca,0,0,L);
...
function tituloformuonco(&$pdf_,&$fila_,&$vec_,&$pag,$m,$pos)
{
	...
	$pdf_->Image('img\enca_formuonco.jpg',2,$fila_,202,0,'','');
