...
if($fila_>250){
  ...
  $pdf_->Image('img\PIE1.JPG',2,265,210,0,'',''); 
	...
	$pdf_->Cell(40,5,$sexo,0);
	$pdf_->SetXY(93,20);
	$pdf_->Cell(37,5,"Contrato:",0);
	$pdf_->SetXY(107,20);
	$pdf_->Cell(40,5,substr($contrato,0,39),0);
	$pdf_->SetXY(173,20);
	...
	if(!empty($horacierre) && $horacierre!='')
	{
		$pdf_->SetXY(90,30);
		$pdf_->Cell(40,5,"Fecha apertura: ".$fecevo."      Fecha cierre: ".$horacierre,0);
	}
	else 
	{	
		$pdf_->SetXY(170,30);
...
$consulta=mysql_query("SELECT codi_usu,id_ing,cod_medi,cod_cie10,tidx_evo,fech_evo,hora_evo,subj_evo,obje_evo,anal_evo,plan_evo,reco_gen,cama_evo,dest_usu,horacierre_evo 
FROM hist_evo 
WHERE iden_evo='$iden_evo'");
$row=mysql_fetch_array($consulta);
$cama=$row[cama_evo];

$barea=mysql_query("SELECT val2_des FROM destipos WHERE codi_des='$row[cama_evo]'");
$row2=mysql_fetch_array($barea);
$rarea=$row2[val2_des];

//Consulto la información del servicio
$consultaser=mysql_query("SELECT nomb_des FROM destipos WHERE codi_des='$rarea'");
...
$horacierre_evo=$row[horacierre_evo];
...
$vec[14]=$horacierre_evo;//=$fecha de cierre de evolucion
...
if($row[tidx_evo]=='1')$tidx_evo='Impresion Diagnostica';
if($row[tidx_evo]=='2')$tidx_evo='Confirmado nuevo';
if($row[tidx_evo]=='3')$tidx_evo='Confirmado repetido';
...
if(mysql_num_rows($consucie)<>0){
  ...
  $pdf->SetXY(5,$fila);
  $pag=titulo($pdf,$fila,$vec,$pag);
  $pdf->Cell(32,5,"Tipo",0);
  $pdf->Cell(12,5,"Codigo",0);
  $pdf->Cell(40,5,"Descripción",0);
  $fila=$fila+4;
  $pdf->SetXY(5,$fila);
  $pag=titulo($pdf,$fila,$vec,$pag);
  $pdf->Cell(33,5,$tidx_evo,0);
  $pdf->Cell(12,5,$row[cod_cie10],0);
...
if(mysql_num_rows($consultadiax)<>0){
  while($rowdiax=mysql_fetch_array($consultadiax)){
    ...
    if(mysql_num_rows($consucie)<>0){
	  ...
	  $pag=titulo($pdf,$fila,$vec,$pag);
	  $pdf->Cell(33,5,"",0);
      $pdf->Cell(12,5,$rowdiax[cod_cie10],0);
      $pdf->Cell(40,5,$rowcie[nom_cie10],0);
	  $fila=$fila+4;
	  $pag=titulo($pdf,$fila,$vec,$pag);
      $pag=titulo($pdf,$fila,$vec,$pag);
...
$fila=$fila+4;
$pag=titulo($pdf,$fila,$vec,$pag);
$consultamed=mysql_query("SELECT henc_med.obse_med, medicamentos2.codi_mdi, hdet_med.dosi_med, hdet_med.unid_med, hdet_med.via_med, hdet_med.frec_med, hdet_med.ufre_med, medicamentos2.nomb_mdi, henc_med.iden_evo
FROM (hdet_med INNER JOIN henc_med ON hdet_med.idor_med = henc_med.idor_med) INNER JOIN medicamentos2 ON hdet_med.codi_mdi = medicamentos2.codi_mdi
WHERE (((henc_med.iden_evo)='$iden_evo'))");
...
if(mysql_num_rows($consultavar)<>0)
{
	while($rowvar=mysql_fetch_array($consultavar))
	{
		...
		$iden_ser=$rowvar[iden_ser];
		if($rowvar[clas_var]<>"I")
		{
			$consultaact=mysql_query("SELECT codi_cup,descrip FROM cups WHERE codigo='$rowvar[iden_ser]'");
			$rowact=mysql_fetch_array($consultaact);
			$iden_ser=$rowact[codi_cup];
...
//PARA CAMBIO EN EL 420 -------------------------------------------------
	$especialidad_var='';
	$consultaEspecialidad=mysql_query("SELECT hist_evo.iden_evo, hist_var.idevo_cumpli, hist_var.especialidad_var, destipos.nomb_des
	FROM (hist_var INNER JOIN destipos ON hist_var.especialidad_var = destipos.codi_des) INNER JOIN hist_evo ON hist_var.idevo_cumpli = hist_evo.iden_evo
	WHERE (((hist_evo.iden_evo)=$iden_evo));");
	while($rowEspe=mysql_fetch_array($consultaEspecialidad))
	{
		$especialidad_var=$rowEspe[nomb_des];
	}
//FIN CAMBIO 420 ---------------------------------------------------------	


$consultamed=mysql_query("SELECT nom_medi,reg_medi, espe_med FROM medicos WHERE cod_medi='$row[cod_medi]'");
if(mysql_num_rows($consultamed)<>0){
  $rowmed=mysql_fetch_array($consultamed);
  $nom_medi=$rowmed[nom_medi];
  $reg_medi=$rowmed[reg_medi];
  $espe_medi=$rowmed[espe_med];
  $bespe=mysql_query("select nomb_des from destipos where codi_des='$espe_medi'");
...
//cambio para el 420 --------------------------------------	
	if ($especialidad_var==''){	
		$fila=$fila+4;
		$pdf->SetXY(100,$fila);
		$pdf->Cell(40,5,$especialidad,0);
	}
	else{
		$fila=$fila+4;
		$pdf->SetXY(100,$fila);
		$pdf->Cell(40,5,$especialidad_var,0);
	} 
//----------------------------------------------------------
...
if(!empty($ingreso)){
	$consulta=mysql_query("SELECT iden_evo FROM hist_evo WHERE id_ing=$ingreso ORDER BY fech_evo, hora_evo");
