...
//$pdf->Cell(40,10,'* - Examen Confirmado ' ,0);
$nu=date("Y-m-d"); 
$ho=strftime("%I:%M:%S");
	...
	INNER JOIN ucontrato AS ucontrato ON ucontrato.CUSU_UCO=usuario.CODI_USU
	INNER JOIN contrato AS contrato ON contrato.CODI_CON=ucontrato.CONT_UCO
	WHERE usuario.CODI_USU='$cod' and ucontrato.ESTA_UCO<>'RE'");
...
if ($bandera=="true")
{
	...
	if($estd_dlab=='P')
	{
        ...
        if(mysql_num_rows($sqlwlab)=='0')
        {
                        ...
                        $result2=mysql_query("SELECT cups.codi_cup,detalle_labs.iden_dlab,detalle_labs.iden_labs, detalle_labs.codigo, cups.descrip, detalle_labs.obsv_dlab,
                        detalle_labs.refe_dlab, detalle_labs.unid_dlab, detalle_labs.cod_medi,detalle_labs.estd_dlab,detalle_labs.etdv_dlab
                        FROM detalle_labs
                                ...
                                if ($bandera=="true")
                                {
                                        ...
                                        if($fila>=190)
                                        {
												...
												*/
                                                ...
                                                if(($rowm[etdv_dlab]=='RE'))
                                                {
                                                        $pdf->setxy(125,$fila);
                                                        $pdf->cell(40,5,"RMTD",0,60);
                                                }           
                                                else
                                                {
                                                        $pdf->setxy(125,$fila);
...
//impresion de datodatos Coprologicos
...
$fila=increm($fila,$pdf,$cod,$num_ord);
$pdf->SetXY(15,$fila);
$pdf->Cell(40,5,"R.Globulos Blancos: ".$gbla_lqd.' /mmￜ',0);

$pdf->SetXY(70,$fila);
$pdf->Cell(40,5,"R.Globulos Rojos: ".$groj_lqd.' /mmￜ',0);
...
if(mysql_num_rows($conotrex)<>0)
{
	...
	{	
		...
		if($cod_loex=='904105')
		{
			...
			$fila=increm($fila+2,$pdf,$cod,$nd_);
			$pdf->SetXY(80,$fila);
			$pdf->Cell(40,5,"HORMONA FOLￜCULO ESTIMULANTE - FSH",0);
		...
		if($cod_loex=='904601')
		{
			...
			$fila=increm($fila,$pdf,$cod,$nd_);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'1 Aￜo              0.12 - 0.21  ',0);

			$fila=increm($fila,$pdf,$cod,$nd_);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'1 - 6 Aￜos       0.03 - 0.32 ',0);

			
			$fila=increm($fila,$pdf,$cod,$nd_);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'7 - 12 Aￜos     0.03 - 0.68',0);

			//resultados
			$fila=increm($fila,$pdf,$cod,$nd_);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'13 - 17 Aￜos   0.28 - 11.1 ',0);
		...
		if($cod_loex=='906446')
		{
			...
			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'1 Aￜo                  Hasta 15',0);
			
			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'1 - 5 Aￜos           Hasta 60',0);

			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'6 - 9 Aￜos           Hasta 90',0);

			
			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila);
			$pdf->Cell(40,5,'10 - 15 Aￜos       Hasta 200',0);
	...
	if($cod_loex=='902219')
		...
		if($cod_loex=='E904004')
		{
			...
			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila+2);
			$pdf->Cell(40,5,'Menos de 50 Años: 450 pg/ml',0);

			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila+2);
			$pdf->Cell(40,5,'De 50 Años: 900 pg/ml',0);
			
			$fila=increm($fila,$pdf,$cod,$num_ord);
			$pdf->SetXY(15,$fila+2);
			$pdf->Cell(40,5,'Mayores 75 Años: 1800 pg/ml',0);
...
/////////////////////////////////////////////////Alcohol Resistente
	...
	FROM labo_oex2 WHERE `iden_dlab`='$iden_labs'  AND `est_oex2`<>'EL' AND `cod_exam`='901101'");
	...
	if(mysql_num_rows($res_alc)<>0)
	{
		...
		while($rowalc= mysql_fetch_array($res_alc))
		{
			...
			if($num_mue=='1')
			{
				...
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nￜ 1  Positivo Para BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
				else
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nￜ 1  No se Observan BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
			...
			if($num_mue=='2')
			{
				...
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nￜ 2  Positivo Para BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
				else
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nￜ 2  No se Observan BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
			...
			if($num_mue=='3')
			{
				...
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nￜ 3  Positivo Para BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
				else
				{
				$pdf->SetXY(15,$fila);
				$pdf->Cell(40,5,'Muestra Nￜ 3  No se Observan BAAR en 100 campos Observados'.'  '.$valo_mue,0);
				}
