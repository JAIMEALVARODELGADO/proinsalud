...
//	$solqui=1;
	...
	*/
	...
	consultaprincipal.inca_cpl, consultaprincipal.feinca_cpl,consultaprincipal.sire_cpl,consultaprincipal.sipi_cpl,
...
*/				
			...
			$fila=$fila+2;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(196,5,'Control en: '.$proxi,1,0,L);
			//$pdf->SetXY(120,$fila);
			//$pdf->Cell(5,5,'Repetir esta formula por '.$repi.' meses',0,0,L);			
			$fila=$fila+5;
			$pdf->SetXY(5,$fila);
			$pdf->MultiCell(196,5,'Recomendaciones: '.$obse,1,L,0);
...
//			$pdf->Cell(15,5,$nuformu,0,0,C);
			...
			$pdf->SetFont('Arial','',6);
			$pdf->Cell($a3,5,'MEDICAMENTOS / FORMA FARMACÉUTICA / CONCETRACIÓN',1,0,C);
			$pdf->SetFont('Arial','',7);
			$pdf->Cell($a4,5,'DOSIS',1,0,C);
			$pdf->Cell($a5,5,'FRECUENCIA',1,0,C);
			$pdf->Cell($a6,5,'VIA ADMINIS',1,0,C);			
...
//------------------------------------- FIN ORIGINAL -----------------------------
			...
			else
			{
				...
				$pdf->Cell(10,5,'Diagnósticos',0,0,L);	
				$pdf->SetXY(25,$fila);
				$pdf->MultiCell(190,4,$vec[10],0,L,0);
				$fila=$pdf->GetY();
			}
			*/			
//------------------------------------- ORIGINAL ----------------------------------------						
...
//-------------------------------------- CAMBIO RESTRICCION DE DESCRIPCION DE CONTENIDO DE DIAGNOSTICO ------------				
			/*
			$bdi2=mysql_query("SELECT diagnosticos2.numc_di2, diagnosticos2.codc_di2, cie_10.nom_cie10,imprimible_cie10 
			FROM diagnosticos2 INNER JOIN cie_10 ON diagnosticos2.codc_di2 = cie_10.cod_cie10
			WHERE (((diagnosticos2.numc_di2)='$numhisto'))");
			$t=1;
			while($rd2=mysql_fetch_array($bdi2))
			{	
				$siimpime2=$rd2['imprimible_cie10'];
				if($siimpime2=='S')
				{
					$desdies2=$rd2['codc_di2'].' - '.$rd2['nom_cie10'];
					$pdf->SetXY(25,$fila);				
...
//fin antimicrobianos	
	...
	if($his=='1')
	{
		...
		*/
		...
		$pdf->Cell(13,4,'Direcciï¿½n:',0,0,L);
		$pdf->Cell(68,4,$diracu,0,0,L);
		$pdf->Cell(13,4,'Telï¿½fono:',0,0,L);
		...
		*/
		...
		$bsoap=mysql_query("select * from consulta_soap where numc_soap='$numhisto'");
		while($rsoap=mysql_fetch_array($bsoap))
		{
			$subjetivo=$rsoap['subj_soap'];
			$objetivo=$rsoap['obje_soap'];
			$analisis=$rsoap['anal_soap'];
			$plan=$rsoap['plan_soap'];
			$exfis=$rsoap['exfi_soap'];		
		}
		
		
		$fila=$fila+5;
		$pdf->SetXY(5,$fila);
		$pdf->Cell(20,4,'Subjetivo:',0,0,L);	
		$pdf->SetXY(35,$fila);
		$pdf->MultiCell(180,4,$subjetivo,0,L,0	);
		$fila=$pdf->GetY();

		$fila=$fila+$h;
		$pdf->SetXY(5,$fila);
		$pdf->Cell(20,4,'Objetivo:',0,0,L);	
		$pdf->SetXY(35,$fila);
		$pdf->MultiCell(180,4,$objetivo,0,L,0);	
		$fila=$pdf->GetY();
		...
		$fila=$fila+$h;
		$pdf->SetXY(5,$fila);
		$pdf->Cell(20,4,'Análisis:',0,0,L);
		$pdf->SetXY(35,$fila);
		$pdf->MultiCell(180,4,$analisis,0,L,0);
		$fila=$pdf->GetY();
	...
	$fila=$fila+$h;
			$pdf->SetXY(5,$fila);
			$pdf->Cell(20,4,'Planificaciï¿½n:',0,0,L);	
			$pdf->SetXY(35,$fila);
	...
	$fila=$fila+$h;
			if($vec[4]=='F')
			{
				...
				while($rfem=mysql_fetch_array($bfem))
				{
					...
					$pdf->Cell(20,4,'Gineco-obstï¿½tricos:',0,0,L);	
					$pdf->SetXY(35,$fila);
					...
					$pdf->Cell(25,4,'Cesï¿½reas: '.$cesar,0,0,L);
	...
	$fila=$fila+$h;
			...
			while($rexaf=mysql_fetch_array($bexafis))
			{
				...
				$pdf->SetXY(5,$fila);			
				$pdf->SetXY(5,$fila);
				$pdf->Cell(23,4,'T.A: '.$tenci1.'/'.$tenci2,0,0,L);
				$pdf->Cell(23,4,'F.R: '.$fres,0,0,L);
				$pdf->Cell(23,4,'F.C: '.$fcar,0,0,L);
				$pdf->Cell(23,4,'T: '.$temp,.0,0,L);			
				$pdf->Cell(23,4,'Peso: '.$peso.' Kg.',0,0,L);
				$pdf->Cell(23,4,'Talla: '.$talla.' Kg.',0,0,L);	
				$pdf->Cell(23,4,'P.C: '.$pcfa,0,0,L);			
				$pdf->Cell(23,4,'IMC: '.$imc,0,0,L);		
				$pdf->Cell(23,4,'ICC: '.$icc,0,0,L);	
			}	
	...
	$fila=$fila+$h;
			...
			$pdf->SetFillColor($col);	
			$pdf->rect(5,$fila,205,4,F);
			$pdf->Cell(20,4,'INFORME PARACLINICOS:',0,0,L);
			$fila=$fila+5;		
			$pdf->SetXY(5,$fila);
			...
			*/
			...
			$pdf->SetXY(5,$fila);
			$pdf->Cell(80,4,'Sintomático respiratorio   '.$sinres,0,0,L);	
	...
	$fila=$fila+$h;
		...
		*/
		...
		if($vec[20]=='878' && $codimedico=='1301')
		{
			$fila=$fila-2;
			$bmed=mysql_query("SELECT * FROM juntamedica_participantes WHERE nhis_cpl='$numhisto'");
			$jm=1;
			while($rmed=mysql_fetch_array($bmed))
			{
				$nommedico_jm=$rmed['nommedico_jm'];
				$espemedico_jm=$rmed['espemedico_jm'];
				$regmedico_jm=$rmed['regmedico_jm'];
				...
				if($jm % 2 != 0)
				{
					...
					$hor=5;
					$pdf->SetXY($hor,$fila);
					$pdf->Cell(10,4,'____________________________________',0,0,L);
					$fila=$fila+4;
					$pdf->SetXY($hor,$fila);
					$pdf->Cell(45,4,$nommedico_jm,0,0,L);
					$fila=$fila+4;
					$pdf->SetXY($hor,$fila);
					$pdf->Cell(45,4,$espemedico_jm,0,0,L);
					$fila=$fila+4;
					$pdf->SetXY($hor,$fila);
					$pdf->Cell(45,4,'Registro medico: '.$regmedico_jm,0,0,L);
		...
		else
		{
			...
			$fila=$fila+4;
			$pdf->SetXY(25,$fila);
			if($regimedico!='')$regimedico='Registro medico: '.$regimedico;
			$pdf->Cell(45,5,$regimedico,0,0,L);
		}
...
//-------------------------				
				...
				$pdf->SetXY(15,$fila);
				$pdf->Cell(50,4,'ANESTESIOLOGO:',0,0,L);
				$pdf->SetXY(49,$fila);	
				$pdf->Cell(50,4,$aneste1,0,0,L);
	...
	if($solqui==1)
	{
		...
		while($rqui=mysql_fetch_array($bussol))		
		{
			for($i=0;$i<2;$i++)
			{
				...
				$idensol=$rqui['iden_solquiro'];			
				$busidensol="SELECT solicitud_quirofano.iden_solquiro, solicitud_quirofano.diag_solquiro, solicitud_quirofano.cups_solquiro, solicitud_quirofano.ticiru_solquiro, solicitud_quirofano.tianes_solquiro, solicitud_quirofano.fecsol_solquiro, solicitud_quirofano.horasol_solquiro, solicitud_quirofano.reayud_solquiro, solicitud_quirofano.sangre_solquiro, solicitud_quirofano.reequi_solquiro, solicitud_quirofano.duracion_solquiro, solicitud_quirofano.reseruci_solquiro, solicitud_quirofano.rematerial_solquiro, solicitud_quirofano.material_solquiro, solicitud_quirofano.origen_solquiro, solicitud_quirofano.relacion_solquiro, encabesadohistoria.cous_ehi, encabesadohistoria.cont_ehi, consultaprincipal.come_cpl
				FROM (solicitud_quirofano INNER JOIN encabesadohistoria ON solicitud_quirofano.nhis_solquiro = encabesadohistoria.numc_ehi) INNER JOIN consultaprincipal ON solicitud_quirofano.nhis_solquiro = consultaprincipal.numc_cpl
				WHERE (((solicitud_quirofano.iden_solquiro)='$idensol'))";
				$resuljusti=Mysql_query($busidensol);
				while($rowsol=mysql_fetch_array($resuljusti))
				...
				$cadcie1=mysql_query("select nom_cie10 from cie_10 where cod_cie10='$diag_solquiro'");
				while ($rowcie1=mysql_fetch_array($cadcie1))
				{
					$nomcie1=$rowcie1['nom_cie10'];
				}
				
				$resulmed=Mysql_query("SELECT descrip,codi_cup FROM cups WHERE codigo='$cups_solquiro'");
				while ($rowmed=mysql_fetch_array($resulmed))
				{
					$nompro=$rowmed['descrip'];
					$codcup=$rowmed['codi_cup'];
				...
				$pdf->SetXY(65,$fila);				
				$pdf->MultiCell(135,4,$losproce32,1,L,0);
				$fila=$pdf->GetY();
				$file2=$fila-$fila1;
				$pdf->SetXY(15,$fila1);
...
//-------------------------				
				...
				$fila1=$fila;
				$pdf->SetXY(65,$fila);				
				$pdf->MultiCell(135,4,$matersqdet.' - '.$losproce32,1,L,0);
				$fila=$pdf->GetY();
				$file2=$fila-$fila1;
				$pdf->SetXY(15,$fila1);
...
//---------------------------	
				...
				$pdf->MultiCell(180,4,'Yo,  '.$nom_cinf.' , Mayor de Edad, Identificado con C.C. No. '.$ident.'  y Como: Paciente_____, o como: Responsable _____,'
				. ' del o la Paciente________________________, identificado(a) con C.C. o T.I. No.______________, Autorizo al Dr.(a). '.$nom_medi.' , con profesion'
				. ' o Especialidad '. $are_medi . '  , para la realizacion del Procedimiento  '. $losproce32,0,1,0);
				$fila=$pdf->GetY()+4;                         
				$pdf->SetXY(15,$fila);						  
				$pdf->MultiCell(180,4,'Que Consiste en:   '.$consissqdet,0,1,0);				
				$fila=$pdf->GetY()+4;                         
				...
				$fila=$fila+$alto+10;
				$pdf->SetXY(15,$fila);					
				$pdf->MultiCell(180,4,'Comprendo y acepto que durante el procedimietno pueden aparecer circustancias imprevisibles o inesperadas que puedan requerir una extension del procedimiento original o la realizacion de otro procedimiento no mencionado arriba.',0,1,0);
				$fila=$pdf->GetY();
				$pdf->SetXY(15,$fila);
...
*/	
	...
	if($inca==1)
	{
		$bin=mysql_query("SELECT incapacidades.id_inca, incapacidades.numc_his, incapacidades.tformato_inca, establecimientos_educativos.nom_depar, 
		establecimientos_educativos.nom_muni, establecimientos_educativos.nombre_establecimiento, incapacidades.ereaespe_inca, incapacidades.jornada_inca, 
		incapacidades.numdias_inca, incapacidades.letdias_inca, incapacidades.fecini_inca, incapacidades.fecfin_inca, incapacidades.dx_inca, incapacidades.obse_inca, 
		incapacidades.tipo_inca, incapacidades.fecparto_inca,
		incapacidades.paciente_inca, incapacidades.tipodoc_inca, incapacidades.docum_inca, usuario.PNOM_USU, 
		usuario.SNOM_USU, usuario.PAPE_USU, usuario.SAPE_USU, incapacidades.sexo_inca, incapacidades.edad_inca, incapacidades.prorroga_inca, medicos.nom_medi, 
		contrato.NEPS_CON, incapacidades.tireg_inca, incapacidades.fecreg_inca, municipio.NOMB_MUN, departamento.NOMB_DEP
		FROM (((((incapacidades LEFT JOIN establecimientos_educativos ON incapacidades.estedu_inca = establecimientos_educativos.iden) LEFT JOIN usuario ON incapacidades.paciente_inca = usuario.CODI_USU) LEFT JOIN medicos ON incapacidades.medico_inca = medicos.cod_medi) LEFT JOIN contrato ON incapacidades.contrato_inca = contrato.CODI_CON) LEFT JOIN municipio ON incapacidades.munate_inca = municipio.CODI_MUN) LEFT JOIN departamento ON municipio.DEPA_MUN = departamento.CODI_DEP
		WHERE (((incapacidades.numc_his)='$numhisto'))");
		while($rin=mysql_fetch_array($bin))
		{
			...
			$nomereaespe_inca=$rin['ereaespe_inca'];
		...
		$pdf->Image('img\pie_inca.JPG',2,123,210,0,'','');
		$fila=$fila+22;
		...
		$pdf->Cell(68,5,$edad_inca,0,0,L);			
			...
			$pdf->SetXY(42,$fila);				
			$pdf->MultiCell(138,5,'  '.$estedu_inca.'  ',0,L,0);
			$pdf->SetFont('Arial','',8);	
			$fila=$pdf->GetY();
...
function titulo(&$pdf_,&$fila_,&$vec_,&$pag,$m,$pos)
{
	...
	if($m==4)
	{	
		$pdf_->Cell(20,5,"Aseguradora:",0);
		$pdf_->SetXY(23,$fila_+25);
