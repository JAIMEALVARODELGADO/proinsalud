...
$ho=strftime("%I:%M:%S");
	...
	$inf=mysql_query("SELECT el.iden_labs, el.codi_usu, el.cod_medi, el.iden_evo, el.iden_cita, el.fchr_labs, el.fche_labs,dl.obsv_dlab, 
...
$result2=Mysql_query("SELECT detalle_labs.iden_dlab,detalle_labs.iden_labs,detalle_labs.codigo, cups.descrip, detalle_labs.obsv_dlab,detalle_labs.etdv_dlab,
detalle_labs.refe_dlab, detalle_labs.unid_dlab, detalle_labs.cod_medi,detalle_labs.estd_dlab,detalle_labs.id_lis
FROM detalle_labs
...
{ 
	...
	$pdf->SetFont('Arial','',9);
	$sql_exa=mysql_query("SELECT detalle_labs.iden_dlab,detalle_labs.iden_labs,detalle_labs.codigo, cups.descrip, detalle_labs.obsv_dlab,detalle_labs.etdv_dlab,
		detalle_labs.refe_dlab, detalle_labs.unid_dlab, detalle_labs.cod_medi,detalle_labs.estd_dlab,detalle_labs.id_lis
		FROM detalle_labs
		INNER JOIN cups AS cups ON detalle_labs.codigo = cups.codigo
		WHERE  detalle_labs.nord_dlab='$nd_' AND detalle_labs.obsv_dlab ='F'");
				
		while ($rowt=mysql_fetch_array($sql_exa)) 
		{
				$fila=$pdf->gety();
				$fila=increm($fila,$pdf,$cod,$nd_);
				$pdf->setxy(15,$fila);
				$pdf->cell(40,5,'FORMATO: '.substr($rowt["descrip"],0,10),0,60);
				
				$cosinter=Mysql_query("SELECT formatos.nombre_examen, formatos.resultado, formatos.valor_ref_min, 
				formatos.valor_ref_max, formatos.unidad, formatos.observaciones, detalle_labs.nord_dlab
				FROM ((cups INNER JOIN detalle_labs ON cups.codigo = detalle_labs.codigo) 
				INNER JOIN encabezado_labs ON detalle_labs.iden_labs = encabezado_labs.iden_labs) 
				INNER JOIN formatos ON detalle_labs.iden_dlab = formatos.iden_dlab
				WHERE formatos.iden_dlab ='$rowt[iden_dlab]'");
				while ($rowcoint = mysql_fetch_array($cosinter)) 
				{
					$fila=$pdf->gety();
					$pdf->setxy(15,$fila);
					$pdf->setxy(15,$fila);
					$pdf->cell(40,5,$rowcoint[nombre_examen] . ' ' .$rowcoint[resultado],0,60);
				}
	...
	$pdf->settextcolor(0,0,0);
	//$pdf->SetFont('Arial','',9);
	/*$cosinter=Mysql_query("SELECT cups.descrip, formatos.nombre_examen, formatos.resultado, formatos.valor_ref_min, 
		formatos.valor_ref_max, formatos.unidad, formatos.observaciones, detalle_labs.nord_dlab
		FROM ((cups INNER JOIN detalle_labs ON cups.codigo = detalle_labs.codigo) 
		INNER JOIN encabezado_labs ON detalle_labs.iden_labs = encabezado_labs.iden_labs) 
		INNER JOIN formatos ON detalle_labs.iden_dlab = formatos.iden_dlab
		WHERE detalle_labs.nord_dlab='$nd_' AND detalle_labs.id_lis<>' ' AND detalle_labs.obsv_dlab ='F'");
		if(mysql_num_rows($cosinter)<>0)
		{
			/*while ($rowcoint = mysql_fetch_array($cosinter)) 
			{
				$fila=$pdf->gety();
				$fila=increm($fila,$pdf,$cod,$nd_);
				$pdf->setxy(15,$fila);
				$pdf->cell(40,5,'FORMATO: '.substr($rowcoint["descrip"],0,10),0,60);
			}*/
			...
			/*while ($rowcoint = mysql_fetch_array($cosinter)) 
			{
				...
				$pdf->setxy(15,$fila);
				$pdf->cell(40,5,$rowcoint[nombre_examen] . ' ' .$rowcoint[resultado],0,60);
				//$pdf->cell(40,5,$rowcoint[nombre_examen] . ' ' .$rowcoint[resultado].' '.$rowcoint[valor_ref_min].' '.$rowcoint[valor_ref_max].' '.$valor_ref_max[observaciones],0,60);
				//$fila=$fila+2;
			}
...
//FILA+4
$fila=increm($fila+6,$pdf,$cod,$nd_);
...
$fila=increm($fila+12,$pdf,$cod,$nd_);
