...
if(!empty($gauditor))
{
	...
	WHERE (((usuario.NROD_USU)='$identificacion') AND ((usuario_auditor.ide_usua)='$gauditor') AND ((ucontrato.ESTA_UCO)='AC'))");
	if(mysql_num_rows($baudi)==0)
	{
		ECHO "<center><h3>El usuario no existe o el auditor no tiene acceso al contrato</h3> </center>";
		exit();
	}
}
 ...
 echo "<tr><th>Opciones</th><th>Nombre</th><th>Fecha</th><th>Profesional</th><th>Servicio</th></tr>";
  while($row=mysql_fetch_array($consulta)){
	...
	if(mysql_num_rows($consultacama)<>0){
	  ...
	  $cama=$rowcama[cama_evo];
      $descama=$rowcama[nomb_des];		  
	  $consultaser=mysql_query("SELECT nomb_des FROM destipos WHERE codi_des = '$cama'");
	  //$rowser=mysql_fetch_array($consultaser);
	  $servicio=$rowser[nomb_des];
	}
	echo "<td bgcolor='#ffffff'>".$descama.' '.$servicio."</td>";
